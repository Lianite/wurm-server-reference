...
public class ToEastChangeStrategy extends AbstractFixedEdgeChangeStrategy
{
    ...
    public ToEastChangeStrategy(final Rectangle2D original, final boolean ratioFixed, final double ratio, final Rectangle2D bounds) {
        super(ratioFixed, ratio, bounds);
        final Point2D edgeCenterPoint = new Point2D(original.getMinX(), (original.getMinY() + original.getMaxY()) / 2.0);
        this.westernEdge = new Edge2D(edgeCenterPoint, Orientation.VERTICAL, original.getMaxY() - original.getMinY());
    }
    
    public ToEastChangeStrategy(final Rectangle2D original, final boolean ratioFixed, final double ratio, final double maxX, final double maxY) {
