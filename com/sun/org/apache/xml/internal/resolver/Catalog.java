...
public class Catalog
{
    ...
    public void addReader(final String mimeType, final CatalogReader reader) {
        if (this.readerMap.containsKey(mimeType)) {
            ...
            final Integer pos = new Integer(this.readerArr.size() - 1);
    ...
    protected void copyReaders(final Catalog newCatalog) {
        ...
        final Enumeration en = this.readerMap.keys();
        while (en.hasMoreElements()) {
            final String mimeType = en.nextElement();
            final Integer pos = this.readerMap.get(mimeType);
            mapArr.set(pos, mimeType);
        }
        for (int count2 = 0; count2 < mapArr.size(); ++count2) {
            final String mimeType2 = mapArr.get(count2);
            final Integer pos2 = this.readerMap.get(mimeType2);
            newCatalog.addReader(mimeType2, this.readerArr.get(pos2));
        }
    }
    ...
    public void parseAllCatalogs() throws MalformedURLException, IOException {
        ...
        while (en.hasMoreElements()) {
            ...
            if (e.getEntryType() == Catalog.DELEGATE_PUBLIC || e.getEntryType() == Catalog.DELEGATE_SYSTEM || e.getEntryType() == Catalog.DELEGATE_URI) {
