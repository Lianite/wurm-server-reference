...
public abstract class JaxBeanInfo<BeanT>
{
    ...
    protected final void setLifecycleFlags() {
        try {
            for (final Method m : this.jaxbType.getDeclaredMethods()) {
                final String name = m.getName();
                if (name.equals("beforeUnmarshal")) {
                    if (this.match(m, JaxBeanInfo.unmarshalEventParams)) {
                    ...
                    if (this.match(m, JaxBeanInfo.marshalEventParams)) {
                        this.cacheLifecycleMethod(m, (short)32);
                    }
                }
                else if (name.equals("afterMarshal") && this.match(m, JaxBeanInfo.marshalEventParams)) {
