...
abstract class PropertyInfoImpl<T, C, F, M> implements PropertyInfo<T, C>, Locatable, Comparable<PropertyInfoImpl>
{
        ...
        if (xjta != null) {
            ...
            if (xjta == null) {
                ...
                if (xsa != null) {
                    ...
                    if (xjta != null) {
                        final T adapter = this.reader().getClassValue(xjta, "value");
                        parent.builder.reportError(new IllegalAnnotationException(Messages.UNMATCHABLE_ADAPTER.format(this.nav().getTypeName(adapter), this.nav().getTypeName(t)), xjta));
                    }
                }
            }
            else {
    ...
    private boolean isApplicable(final XmlJavaTypeAdapter jta, final T declaredType) {
        ...
        final T adapter = this.reader().getClassValue(jta, "value");
        final T ba = this.nav().getBaseClass(adapter, this.nav().asDecl(XmlAdapter.class));
