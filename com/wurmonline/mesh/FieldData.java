...
public final class FieldData
{
    ...
    private static final String[] fieldAges;
    
    public static String getFieldName(final int fieldType) {
        switch (fieldType) {
            case 0: {
                return "Barley";
            }
    ...
    public static String getModelResourceName(final int fieldType) {
        return "img.texture.crop." + getFieldName(fieldType).toLowerCase().replace(" ", "");
    }
    
    public static String getHelpSubject(final int fieldType) {
        return "Terrain:" + getFieldName(fieldType).replace(' ', '_');
    }
    
    public static int getType(final Tiles.Tile fieldType, final byte data) {
    ...
    public static int getAge(final byte data) {
        return (data & 0x70) >> 4;
    }
    
    public static boolean isTended(final byte data) {
        return (data & 0x80) != 0x0;
    }
    
    public static String getTypeName(final Tiles.Tile fieldType, final byte data) {
        return getFieldName(getType(fieldType, data));
    }
    
    public static String getAgeName(final byte data) {
        return FieldData.fieldAges[getAge(data)];
    }
    
    static {
        fieldAges = new String[] { "freshly sown", "sprouting", "growing", "halfway", "almost ripe", "ripe", "ripe", "only weeds" };
    }
}
