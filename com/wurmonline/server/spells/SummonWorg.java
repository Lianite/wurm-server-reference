...
public class SummonWorg extends KarmaSpell implements CreatureTemplateIds
{
    ...
    private final void spawnCreature(final int templateId, final Creature performer) {
        try {
            ...
            SummonWorg.logger.log(Level.WARNING, nst.getMessage(), nst);
        }
        catch (Exception ex) {
            SummonWorg.logger.log(Level.WARNING, ex.getMessage(), ex);
        }
    }
    
    static {
        logger = Logger.getLogger(SummonWorg.class.getName());
    }
}
