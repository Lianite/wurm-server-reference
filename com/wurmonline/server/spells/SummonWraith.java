...
public class SummonWraith extends KarmaSpell implements CreatureTemplateIds
{
    ...
    private final void spawnCreature(final int templateId, final Creature performer) {
        try {
            ...
            SummonWraith.logger.log(Level.WARNING, nst.getMessage(), nst);
        }
        catch (Exception ex) {
            SummonWraith.logger.log(Level.WARNING, ex.getMessage(), ex);
        }
    }
    
    static {
        logger = Logger.getLogger(SummonWraith.class.getName());
    }
}
