...
public class Summon extends KarmaSpell implements CreatureTemplateIds
{
    ...
    void doEffect(final Skill castSkill, final double power, final Creature performer, final int tilex, final int tiley, final int layer, final int heightOffset) {
        try {
            ...
            Summon.logger.log(Level.WARNING, ex.getMessage(), ex);
        }
    }
    ...
    private final void spawnCreature(final int templateId, final Creature performer) {
        try {
            ...
            Creature.doNew(templateId, true, performer.getPosX() - 4.0f + Server.rand.nextFloat() * 9.0f, performer.getPosY() - 4.0f + Server.rand.nextFloat() * 9.0f, Server.rand.nextFloat() * 360.0f, performer.getLayer(), ct.getName(), sex, performer.getKingdomId(), ctype, true);
        }
        catch (NoSuchCreatureTemplateException nst) {
            Summon.logger.log(Level.WARNING, nst.getMessage(), nst);
        }
        catch (Exception ex) {
            Summon.logger.log(Level.WARNING, ex.getMessage(), ex);
        }
    ...
    static {
        logger = Logger.getLogger(Summon.class.getName());
    }
}
