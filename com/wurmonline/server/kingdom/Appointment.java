...
public final class Appointment implements MiscConstants
{
    ...
    private static final Map<Integer, Appointment> jennapps;
    private static final Map<Integer, Appointment> hotsapps;
    private static final Map<Integer, Appointment> molrapps;
    private static final Map<Integer, Appointment> nonerapps;
    
    private static void addAppointment(final Appointment app) {
        final Kingdom k = Kingdoms.getKingdom(app.kingdom);
        if (app.kingdom == 1 || k.getTemplate() == 1) {
            Appointment.jennapps.put(app.id, app);
        }
        else if (app.kingdom == 3 || k.getTemplate() == 3) {
            Appointment.hotsapps.put(app.id, app);
        }
        else if (app.kingdom == 2 || k.getTemplate() == 2) {
            Appointment.molrapps.put(app.id, app);
        }
        else {
            Appointment.nonerapps.put(app.id, app);
        }
    }
    
    public static final Appointment getAppointment(final int id, final byte kingdom) {
        final Kingdom k = Kingdoms.getKingdom(kingdom);
        if (kingdom == 1 || k.getTemplate() == 1) {
            return Appointment.jennapps.get(id);
        }
        if (kingdom == 3 || k.getTemplate() == 3) {
            return Appointment.hotsapps.get(id);
        }
        if (kingdom == 2 || k.getTemplate() == 2) {
            return Appointment.molrapps.get(id);
        }
        return Appointment.nonerapps.get(id);
    }
    
    static void setAppointments(final Appointments app) {
    ...
    private Appointment(final String _malename, final String _femalename, final int _id, final byte _kingdom, final byte _type, final int _level) {
    ...
    public String getNameForGender(final byte gender) {
        if (gender == 1) {
    ...
    public boolean equals(final Object obj) {
        ...
        return this.id == other.id && this.kingdom == other.kingdom;
    }
    
    @Override
    public String toString() {
        final StringBuilder lBuilder = new StringBuilder(150);
    ...
    static {
        jennapps = new HashMap<Integer, Appointment>();
        hotsapps = new HashMap<Integer, Appointment>();
        molrapps = new HashMap<Integer, Appointment>();
        nonerapps = new HashMap<Integer, Appointment>();
