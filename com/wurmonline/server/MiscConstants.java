...
public interface MiscConstants
{
    ...
    public static final byte SEX_NOSEX = Byte.MAX_VALUE;
    ...
    default String getDirectionString(final byte direction) {
        switch (direction) {
            case 0: {
                return "north";
            }
    ...
    default BitSet createBitSetLong(final long fromLong) {
        final BitSet bitSet = new BitSet(64);
        for (int x = 0; x < 64; ++x) {
            if (x == 0) {
                if ((fromLong & 0x1L) == 0x1L) {
                    bitSet.set(x, true);
                }
                else {
                    bitSet.set(x, false);
    ...
    default long bitSetToLong(final BitSet bitSet) {
        long ret = 0L;
        for (int x = 0; x < 64; ++x) {
            if (bitSet.get(x)) {
                ret += 1L << x;
