...
public class WelcomeStage extends TutorialStage
{
    ...
    public WelcomeStage(final long playerId) {
        super(playerId);
    }
    ...
    public class WelcomeSubStage extends TutorialSubStage
    {
        public WelcomeSubStage(final long playerId) {
            super(playerId);
        }
        
        @Override
        protected void buildBMLString() {
            final BMLBuilder builder = BMLBuilder.createBMLBorderPanel(BMLBuilder.createCenteredNode(BMLBuilder.createVertArrayNode(false).addText("").addHeader("Welcome to Wurm!", Color.LIGHT_GRAY)), null, BMLBuilder.createVertArrayNode(false).addPassthrough("tutorialid", Long.toString(this.getPlayerId())).addText("\r\nThis tutorial will show you how to get started in Wurm.\r\n\r\nClick [Next] below to get started.", null, null, null, 300, 400).addText(""), null, BMLBuilder.createLeftAlignedNode(BMLBuilder.createHorizArrayNode(false).addButton("back", "Back", 80, 20, false).addText("", null, null, null, 35, 0).addButton("next", "Next", 80, 20, true).maybeAddSkipButton(Players.getInstance().getPlayerOrNull(this.getPlayerId()), true)));
