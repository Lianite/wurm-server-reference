...
public final class Vehicles implements ProtoConstants, CreatureTemplateIds
{
    ...
    public static Vehicle getVehicle(final Item item) {
        Vehicle toReturn = null;
        if (item.isVehicle() || item.isHitchTarget()) {
            toReturn = Vehicles.vehicles.get(item.getWurmId());
            if (toReturn == null) {
    ...
    public static Vehicle getVehicle(final Creature creature) {
        Vehicle toReturn = null;
        if (creature.isVehicle()) {
            toReturn = Vehicles.vehicles.get(creature.getWurmId());
            if (toReturn == null) {
    ...
    public static Vehicle getVehicleForId(final long id) {
        return Vehicles.vehicles.get(id);
    }
    
    public static Vehicle createVehicle(final Item item) {
        final Vehicle vehic = new Vehicle(item.getWurmId());
        setSettingsForVehicle(item, vehic);
        Vehicles.vehicles.put(item.getWurmId(), vehic);
        return vehic;
    }
    
    public static Vehicle createVehicle(final Creature creature) {
        final Vehicle vehic = new Vehicle(creature.getWurmId());
        setSettingsForVehicle(creature, vehic);
        Vehicles.vehicles.put(creature.getWurmId(), vehic);
        return vehic;
    }
    
    static void setSettingsForVehicle(final Creature creature, final Vehicle vehicle) {
        final int cid = creature.getTemplate().getTemplateId();
    ...
    static void setSettingsForVehicle(final Item item, final Vehicle vehicle) {
        if (item.getTemplateId() == 861) {
            ...
            final Seat[] hitches = { new Seat((byte)2) };
            hitches[0].offx = -3.0f;
            hitches[0].offy = -1.0f;
        ...
        if (item.getTemplateId() == 186) {
            ...
            vehicle.commandType = 2;
            final Seat[] hitches = { new Seat((byte)2), new Seat((byte)2) };
            hitches[0].offx = -2.0f;
            hitches[0].offy = -1.0f;
            hitches[1].offx = -2.0f;
            hitches[1].offy = 1.0f;
            ...
            vehicle.commandType = 2;
            final Seat[] hitches = { new Seat((byte)2), new Seat((byte)2), new Seat((byte)2), new Seat((byte)2) };
            hitches[0].offx = -2.0f;
            hitches[0].offy = -1.0f;
            hitches[1].offx = -2.0f;
            hitches[1].offy = 1.0f;
    ...
    public static void destroyVehicle(final long id) {
        final Vehicle vehicle = Vehicles.vehicles.get(id);
        if (vehicle != null) {
            vehicle.kickAll();
    ...
    static int getNumberOfVehicles() {
        ...
        Vehicles.logger.warning("vehicles Map is null");
        return 0;
    }
    
    public static final void removeDragger(final Creature dragger) {
        for (final Vehicle draggedVehicle : Vehicles.vehicles.values().toArray(new Vehicle[Vehicles.vehicles.size()])) {
    ...
    static {
        logger = Logger.getLogger(Vehicles.class.getName());
        Vehicles.vehicles = new ConcurrentHashMap<Long, Vehicle>();
    }
}
