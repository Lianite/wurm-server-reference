...
final class PlanetBehaviour extends Behaviour implements MiscConstants
{
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item object, final int planetId) {
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final int planetId) {
        final List<ActionEntry> toReturn = new LinkedList<ActionEntry>();
    ...
    @Override
    public boolean action(final Action action, final Creature performer, final int planetId, final short num, final float counter) {
        if (num == 1) {
            if (planetId == 0) {
    ...
    @Override
    public boolean action(final Action action, final Creature performer, final Item source, final int planetId, final short num, final float counter) {
        if (num == 1) {
        ...
        if (num == 118) {
            ...
            if (!performer.isOnSurface()) {
                ...
                final int timePerStage = insta ? 2 : 10;
    ...
    static final String getName(final int planetId) {
        if (planetId == 0) {
            return "Jackal";
        }
        if (planetId == 1) {
