...
public enum Herb
{
    ...
    private Herb(final byte aTileType, final GrassData.GrowthStage aGrassLength, final short aCategory, final int aItemType, final byte aMaterial, final int aChanceAt1, final int aChanceAt100, final int aDifficultyAt1, final int aDifficultyAt100, final ModifiedBy aModifiedBy, final int aChanceModifier) {
    ...
    public static Herb getRandomHerb(final Creature performer, final byte aTileType, final GrassData.GrowthStage aGrassLength, final short aCategory, final int knowledge, final int tilex, final int tiley) {
        ...
        for (final Herb h : values()) {
            if (h.tileType == checkType && h.grassLength == aGrassLength) {
                final float chance = h.getChanceAt(performer, knowledge, tilex, tiley);
                if (chance >= 0.0f) {
                    totalChance += chance;
                }
            }
        ...
        for (final Herb h2 : values()) {
            if (h2.tileType == checkType && (checkType != Tiles.Tile.TILE_GRASS.id || h2.grassLength == aGrassLength)) {
                final float chance2 = h2.getChanceAt(performer, knowledge, tilex, tiley);
                if (chance2 >= 0.0f) {
                    runningChance += chance2;
                    if (rndChance < runningChance) {
                        if (aCategory == 224 || aCategory == h2.category) {
                            return h2;
                        }
                        return null;
                    }
