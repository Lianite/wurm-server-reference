...
final class WoundBehaviour extends Behaviour implements ItemMaterials, MiscConstants
{
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Wound target) {
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item source, final Wound target) {
        final List<ActionEntry> toReturn = new LinkedList<ActionEntry>();
        ...
        if (performer.getDeity() != null && source.isHolyItem(performer.getDeity()) && (performer.isPriest() || performer.getPower() > 0)) {
            ...
            final Spell[] spells = performer.getDeity().getSpellsTargettingWounds((int)faith);
            if (spells.length > 0) {
    ...
    @Override
    public boolean action(final Action act, final Creature performer, final Wound target, final short action, final float counter) {
        final boolean done = true;
        final Creature owner = target.getCreature();
    ...
    @Override
    public boolean action(final Action act, final Creature performer, final Item source, final Wound target, final short action, final float counter) {
        final boolean done = true;
        final Creature owner = target.getCreature();
        if (owner != null && !performer.isWithinDistanceTo(owner.getPosX(), owner.getPosY(), owner.getPositionZ() + owner.getAltOffZ(), 8.0f)) {
            return true;
