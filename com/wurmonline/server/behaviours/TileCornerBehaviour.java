...
final class TileCornerBehaviour extends Behaviour implements MiscConstants
{
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item source, final int tilex, final int tiley, final boolean onSurface, final boolean corner, final int tile, final int heightOffset) {
        ...
        else if (Features.Feature.HIGHWAYS.isEnabled() && source.isRoadMarker() && !Zones.getOrCreateTile(tilex, tiley, onSurface).hasFenceOnCorner(performer.getFloorLevel()) && performer.isWithinDistanceTo(tilex << 2, tiley << 2, 0.0f, 4.0f) && passFloorCheck(tilex, tiley, onSurface, heightOffset)) {
            final HighwayPos highwayPos = MethodsHighways.getHighwayPos(tilex, tiley, onSurface, heightOffset);
            if (highwayPos != null && ((MethodsHighways.middleOfHighway(highwayPos) && !MethodsHighways.containsMarker(highwayPos, (byte)0) && !MethodsHighways.isNextToACamp(highwayPos)) || performer.getPower() > 1)) {
    ...
    @Override
    public List<ActionEntry> getBehavioursFor(final Creature performer, final int tilex, final int tiley, final boolean onSurface, final boolean corner, final int tile, final int heightOffset) {
        final List<ActionEntry> toReturn = new LinkedList<ActionEntry>();
    ...
    @Override
    public boolean action(final Action act, final Creature performer, final Item source, final int tilex, final int tiley, final boolean onSurface, final boolean corner, final int tile, final int heightOffset, final short action, final float counter) {
        boolean done = true;
        if (action == 1) {
            ...
            if (onSurface) {
                ...
                final TilePos digTilePos = TilePos.fromXY(tilex, tiley);
    ...
    @Override
    public boolean action(final Action act, final Creature performer, final int tilex, final int tiley, final boolean onSurface, final boolean corner, final int tile, final int heightOffset, final short action, final float counter) {
        if (action == 1) {
            ...
            if (performer.getPower() > 2) {
                final int meshtile = Server.surfaceMesh.getTile(tilex, tiley);
                ...
                final int currtile = Server.caveMesh.getTile(tilex, tiley);
    ...
    private static boolean passFloorCheck(final int tilex, final int tiley, final boolean onSurface, final int heightOffset) {
        if (heightOffset != 0) {
            return true;
        }
        Floor[] floors = null;
