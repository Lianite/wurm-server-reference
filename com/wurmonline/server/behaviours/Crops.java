...
public final class Crops implements MiscConstants
{
    ...
    public static final String getCropName(final int cropNumber) {
        String cropString = "Unknown crop";
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getNumber() == cropNumber) {
                cropString = Crops.cropTypes[x].getCropName();
                return cropString;
            }
        }
        return cropString;
    ...
    public static final byte getTileType(final int cropNumber) {
        if (cropNumber < 16) {
    ...
    static final int getItemTemplate(final int cropNumber) {
        int itemTemplate = -10;
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getNumber() == cropNumber) {
                itemTemplate = Crops.cropTypes[x].getTemplateId();
                return itemTemplate;
            }
        }
        return itemTemplate;
    ...
    static final int getProductTemplate(final int cropNumber) {
        int productTemplate = -10;
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getNumber() == cropNumber) {
                productTemplate = Crops.cropTypes[x].getProductId();
    ...
    static final int getNumber(final int templateId) {
        int cropNumber = -10;
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getTemplateId() == templateId) {
                cropNumber = Crops.cropTypes[x].getNumber();
                return cropNumber;
            }
        }
        return cropNumber;
    ...
    public static final int getCropNumber(final byte tileType, final byte tileData) {
        if (tileType == 7) {
            return tileData & 0xF;
        }
    ...
    public static final byte encodeFieldData(final boolean tended, final int fieldAge, final int cropNumber) {
        return (byte)((tended ? 128 : 0) + (fieldAge << 4) + (cropNumber & 0xF));
    }
    
    public static final byte decodeFieldAge(final byte data) {
        return (byte)(data >> 4 & 0x7);
    }
    
    public static final boolean decodeFieldState(final byte data) {
        return (data & 0x80) != 0x0;
    }
    
    static final String getMeasure(final int cropNumber) {
        String cropString = "";
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getNumber() == cropNumber) {
                cropString = Crops.cropTypes[x].getMeasure();
                return cropString;
            }
        }
        return cropString;
    ...
    static final double getDifficultyFor(final int cropNumber) {
        double diff = 10.0;
        for (int x = 0; x < Crops.cropTypes.length; ++x) {
            if (Crops.cropTypes[x].getNumber() == cropNumber) {
                diff = Crops.cropTypes[x].getDifficulty();
                return diff;
            }
        }
        return diff;
