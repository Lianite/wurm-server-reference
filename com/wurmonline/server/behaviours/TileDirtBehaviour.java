...
final class TileDirtBehaviour extends TileBehaviour
{
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item subject, final int tilex, final int tiley, final boolean onSurface, final int tile) {
        ...
        if (subject.isSeed()) {
            ...
            toReturn.add(new ActionEntry((short)(-2), "Lay boards", "paving", TileDirtBehaviour.emptyIntArr));
    ...
    public boolean action(final Action act, final Creature performer, final Item source, final int tilex, final int tiley, final boolean onSurface, final int heightOffset, final int tile, final short action, final float counter) {
        ...
        if (action == 153 && source.isSeed()) {
            ...
            try {
                ...
                final boolean isWaterPlant = source.getTemplateId() == 744 || source.getTemplateId() == 746;
            ...
            if (counter == 1.0f) {
                ...
                final short time = (short)(130.0 - farming.getKnowledge(source, 0.0) - act.getRarity() - source.getRarity());
                performer.sendActionControl(Actions.actionEntrys[153].getVerbString(), true, time);
                act.setTimeLeft(time);
            }
            else if (counter > act.getTimeLeft() / 10) {
                if (act.getRarity() != 0) {
                    performer.playPersonalSound("sound.fx.drumroll");
                }
                performer.getStatus().modifyStamina(-2000.0f);
                done = true;
                final int crop = Crops.getNumber(source.getTemplateId());
                final byte type = Tiles.decodeType(tile);
                if (type == Tiles.Tile.TILE_DIRT.id) {
                    if (onSurface) {
                        ...
                        final Skill farming2 = performer.getSkills().getSkillOrLearn(10049);
                        farming2.skillCheck(diff, 0.0, false, 1.0f);
                        Players.getInstance().sendChangedTile(tilex, tiley, onSurface, false);
                        final int resource = (int)(100.0 - farming2.getKnowledge(0.0) + source.getQualityLevel() + source.getRarity() * 20 + act.getRarity() * 50);
