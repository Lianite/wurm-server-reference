...
{
    private static List<ActionEntry> defaultItemActions;
    private static List<ActionEntry> defaultCreatureActions;
    private static List<ActionEntry> defaultTileActions;
    private static List<ActionEntry> defaultStructureActions;
    private static List<ActionEntry> defaultWoundActions;
    private static List<ActionEntry> defaultTileBorderActions;
    private static List<ActionEntry> defaultPlanetActions;
    ...
    public static final ActionEntry[] actionEntrys;
    
    public static String getVerbForAction(final short action) {
        return Actions.actionEntrys[action].getVerbString();
    }
    
    public static String getActionString(final short action) {
        return Actions.actionEntrys[action].getActionString();
    }
    
    public static String createRandomString() {
        final Random rand = new Random();
        final int length = rand.nextInt(3) + 5;
        final char[] password = new char[length];
        for (int x = 0; x < length; ++x) {
            final int randDecimalAsciiVal = rand.nextInt("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".length());
    ...
    private static void createDefaultCreatureActions() {
        Actions.defaultCreatureActions.add(new ActionEntry((short)(-1), "Look", "look", Actions.EMPTY_INT_ARRAY));
        Actions.defaultCreatureActions.add(Actions.actionEntrys[2]);
    }
    
    private static void createDefaultTileActions() {
        Actions.defaultTileActions.add(Actions.actionEntrys[109]);
    }
    
    static List<ActionEntry> getDefaultItemActions() {
        return Actions.defaultItemActions;
    }
    
    static List<ActionEntry> getDefaultCreatureActions() {
        return Actions.defaultCreatureActions;
    }
    
    static List<ActionEntry> getDefaultTileActions() {
        return Actions.defaultTileActions;
    }
    
    static List<ActionEntry> getDefaultTileBorderActions() {
        return Actions.defaultTileBorderActions;
    }
    
    static List<ActionEntry> getDefaultPlanetActions() {
        return Actions.defaultPlanetActions;
    }
    
    static List<ActionEntry> getDefaultStructureActions() {
        return Actions.defaultStructureActions;
    }
    
    static List<ActionEntry> getDefaultWoundActions() {
        return Actions.defaultWoundActions;
    }
    
    static final float getStaminaModiferFor(final Creature performer, final int staminaNeeded) {
        final int currstam = performer.getStatus().getStamina();
        float staminaMod = 1.0f;
        if (currstam > 60000) {
            staminaMod = 0.8f;
        }
        staminaMod += 1.0f - currstam / 65535.0f;
        if (currstam < staminaNeeded) {
            final float diff = staminaNeeded - currstam;
            staminaMod += diff / staminaNeeded * performer.getStaminaMod();
        }
        return staminaMod;
    }
    
    public static final int getStandardActionTime(final Creature performer, final Skill skill, @Nullable final Item source, final double bonus) {
        double knowl = 1.0;
        if (source == null) {
        ...
        double time = (100.0 - knowl) * multiplier;
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = 30.0 + time * (1.0 - 0.2 * source.getSpellSpeedBonus() / 100.0);
        }
        else {
            time += 30.0;
        }
        if (source != null) {
            time -= getRarityTimeModifier(source.getRarity());
            if (source.getSpellEffects() != null) {
                final float modifier = 1.0f / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
                time *= modifier;
            }
        }
        time *= getStaminaModiferFor(performer, 20000);
        time = Math.min(65535.0, time);
        return (int)time;
    }
    
    public static final int getQuickActionTime(final Creature performer, final Skill skill, @Nullable final Item source, final double bonus) {
        double knowl = 1.0;
        if (source == null) {
            knowl = skill.getKnowledge(0.0);
        }
        ...
        double time = (100.0 - knowl) / Servers.localServer.getActionTimer();
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = 30.0 + time * (1.0 - 0.5 * source.getSpellSpeedBonus() / 100.0);
        }
        else {
            time += 30.0;
        }
    ...
    public static final int getVariableActionTime(final Creature performer, final Skill skill, @Nullable final Item source, final double bonus, final int max, final int min, final int staminaNeeded) {
        final double skillVal = (source == null) ? skill.getKnowledge(bonus) : skill.getKnowledge(source, bonus);
        final float multiplier = (max - min) / 100.0f / Servers.localServer.getActionTimer();
        double time = (100.0 - skillVal) * multiplier;
        if (source != null) {
            if (source.getSpellSpeedBonus() > 0.0f) {
                time *= 1.0f - source.getSpellSpeedBonus() / 100.0f * 0.5f;
            }
            if (source.getRarity() > 0) {
                time *= 1.0f - getRarityTimeModifier(source.getRarity());
            }
            if (source.getSpellEffects() != null) {
                time *= 1.0f / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
            }
        }
        time *= getStaminaModiferFor(performer, staminaNeeded);
        time += min;
        return Math.min((int)time, 65535);
    }
    
    public static final int getSlowActionTime(final Creature performer, final Skill skill, @Nullable final Item source, final double bonus) {
        double knowl = 1.0;
        if (source == null) {
            knowl = skill.getKnowledge(0.0);
        }
        ...
        final float multiplier = 3.0f / Servers.localServer.getActionTimer();
        double time = Math.max(20.0, (100.0 - knowl) * multiplier);
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = 25.0 + time * (1.0 - 0.5 * source.getSpellSpeedBonus() / 100.0);
        }
        else {
            time += 25.0;
        }
    ...
    public static final int getPickActionTime(final Creature performer, final Skill skill, final Item source, final double bonus) {
        double knowl = 1.0;
        if (source == null) {
            knowl = skill.getKnowledge(0.0);
        }
        ...
        int time = (int)(Math.max(300.0, (100.0 - knowl) * 5.0) / Servers.localServer.getActionTimer());
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = (int)Math.max(200.0, time * (1.0 - 0.5 * source.getSpellSpeedBonus() / 100.0));
        }
        if (source != null) {
            time -= (int)(getRarityTimeModifier(source.getRarity()) * 10.0f);
            if (source.getSpellEffects() != null) {
                final float modifier = 1.0f / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
                time *= (int)modifier;
            }
        }
        time *= (int)getStaminaModiferFor(performer, 5000);
        time = Math.min(65535, time);
        return time;
    }
    
    public static final int getMoveActionTime(final Creature performer) {
    ...
    public static final int getLoadUnloadActionTime(final Creature performer) {
        int time = 50;
        if (canReceiveDeedSpeedBonus(performer)) {
            time /= 5;
        }
    ...
    public static final int getPlantActionTime(final Creature performer, final Item item) {
        int time = 100;
        if (canReceiveDeedSpeedBonus(performer)) {
            time /= 5;
        }
            ...
            final double knowledge = paving.getKnowledge(0.0);
            final int newTime = (int)(120.0 - knowledge);
    ...
    public static final boolean canReceiveDeedSpeedBonus(final Creature performer) {
        final Village currentVillage = performer.currentVillage;
    ...
    public static final float getRarityTimeModifier(final byte rarity) {
        switch (rarity) {
            case 0: {
                return 0.0f;
            }
    ...
    public static final int getImproveActionTime(final Creature performer, final Item source) {
        int time = (int)(150.0f / Servers.localServer.getActionTimer());
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = (int)Math.max(50.0, time * (1.0 - 0.5 * source.getSpellSpeedBonus() / 100.0));
        }
        if (source != null) {
            time -= (int)(getRarityTimeModifier(source.getRarity()) * 10.0f);
            if (source.getSpellEffects() != null) {
        ...
        time *= (int)getStaminaModiferFor(performer, 20000);
        time = Math.min(65535, time);
        time = Math.max(15, time);
        return time;
    }
    
    public static final int getRepairActionTime(final Creature performer, final Skill skill, final double bonus) {
        int time = (int)(80.0 * (1.0 + (100.0 - skill.getKnowledge(bonus)) / 100.0));
        time *= (int)getStaminaModiferFor(performer, 20000);
        time /= (int)Servers.localServer.getActionTimer();
    ...
    public static final int getDestroyActionTime(final Creature performer, final Skill skill, final Item source, final double bonus) {
        double knowl = 1.0;
        if (source == null) {
            knowl = skill.getKnowledge(0.0);
        }
        ...
        int time = (int)(Math.max(300.0, (100.0 - knowl) * 5.0) / Servers.localServer.getActionTimer());
        if (source != null && source.getSpellSpeedBonus() > 0.0f) {
            time = (int)(Math.max(50.0, time * (1.0 - 0.5 * source.getSpellSpeedBonus() / 100.0)) / Servers.localServer.getActionTimer());
        }
        if (source != null) {
            time -= (int)getRarityTimeModifier(source.getRarity());
            if (source.getSpellEffects() != null) {
                final float modifier = 1.0f / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
                time *= (int)modifier;
            }
    ...
    public static final int getItemCreationTime(final int startTime, final Creature performer, final Skill primSkill, final CreationEntry entry, final Item realSource, final Item realTarget, final boolean isMassProduction) throws NoSuchTemplateException {
        final float standardTime = 100.0f / Servers.localServer.getActionTimer();
        final float difficultyTimeMod = Math.max(0.9f, (startTime - entry.getDifficultyFor(realTarget, realSource, performer)) / 100.0f);
        float minTime = 60.0f;
        if (isMassProduction) {
            minTime = 50.0f;
        }
        double time = minTime + (standardTime - primSkill.getKnowledge(realSource, 0.0)) * difficultyTimeMod;
        if (realSource != null && realSource.getSpellSpeedBonus() > 0.0f) {
            time *= 1.0 - 0.2 * realSource.getSpellSpeedBonus() / 100.0;
        }
        else if (realTarget != null && realTarget.getSpellSpeedBonus() > 0.0f) {
            time *= 1.0 - 0.2 * realTarget.getSpellSpeedBonus() / 100.0;
        }
        if (realTarget != null) {
            time -= getRarityTimeModifier(realTarget.getRarity());
            if (realTarget.getSpellEffects() != null) {
                final float modifier = 1.0f / realTarget.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
                time *= modifier;
            }
        }
        if (realSource != null) {
            time -= getRarityTimeModifier(realSource.getRarity());
            if (realSource.getSpellEffects() != null) {
                final float modifier = 1.0f / realSource.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
                time *= modifier;
            }
        }
        time *= getStaminaModiferFor(performer, 20000);
    ...
    public static final int getRecipeCreationTime(final int startTime, final Creature performer, final Skill primSkill, final Recipe entry, final Item realSource, final Item realTarget, final boolean isMassProduction) throws NoSuchTemplateException {
        final float standardTime = 100.0f / Servers.localServer.getActionTimer();
        final float difficultyTimeMod = Math.max(0.9f, (startTime - entry.getChanceFor(realSource, realTarget, performer)) / 100.0f);
        float minTime = 30.0f;
        if (isMassProduction) {
            minTime = 25.0f;
        }
    ...
    public static final boolean isMultipleRunEpicAction(final int action) {
        return action == 491 || action == 492 || action == 148 || action == 142 || action == 47;
    }
    
    public static final short getCorrectActionByTargetType(final short action, final long targetType) {
        switch (action) {
            case 162:
            case 193: {
    ...
    public static final short getCorrectRepairActionByTargetType(final long targetType) {
    ...
    public static final boolean isMultipleItemAction(final short action, final long[] targets) {
    ...
    public static final boolean isActionBrand(final short action) {
        return action == 484 || action == 643;
    }
    
    public static final boolean isActionBreed(final short action) {
        return action == 379;
    }
    
    public static final boolean isActionGroom(final short action) {
        return action == 398;
    }
    
    public static final boolean isActionBuildFence(final short action) {
    ...
    public static final boolean isActionBuild(final short action) {
        return action == 116 || action == 231 || action == 232 || action == 363 || action == 169 || action == 170 || action == 516 || action == 517 || action == 521 || isActionBuildFence(action);
    }
    
    public static final boolean isActionButcher(final short action) {
        return action == 120;
    }
    
    public static final boolean isActionChop(final short action) {
        return action == 96 || action == 468;
    }
    
    public static final boolean isActionCultivate(final short action) {
        return action == 318;
    }
    
    public static final boolean isActionDestroy(final int action) {
        return action == 171 || action == 524 || action == 174 || action == 175 || action == 172 || action == 173 || action == 82 || action == 57 || action == 757;
    }
    
    public static final boolean isActionDestroyFence(final short action) {
        return action == 171 || action == 172 || action == 173;
    }
    
    public static final boolean isActionDestroyItem(final short action) {
        return action == 83 || action == 757;
    }
    
    public static final boolean isActionDietySpell(final short action) {
        return action == 245;
    }
    
    public static final boolean isActionDig(final short action) {
        return action == 144 || action == 362 || action == 921 || action == 927;
    }
    
    public static final boolean isActionDrop(final short action) {
        return action == 7;
    }
    
    public static final boolean isActionPlantItem(final short action) {
        return action == 176 || action == 746 || action == 747;
    }
    
    public static final boolean isActionFarm(final short action) {
        return action == 151;
    }
    
    public static final boolean isActionForageBotanizeInvestigate(final short action) {
    ...
    public static final boolean isActionGather(final short action) {
        return action == 645;
    }
    
    public static final boolean isActionPick(final short action) {
        return action == 137 || action == 852;
    }
    
    public static final boolean isActionHarvest(final short action) {
        return action == 152;
    }
    
    public static final boolean isActionImproveOrRepair(final short action) {
        return action == 192 || action == 162 || action == 193;
    }
    
    public static final boolean isActionLead(final short action) {
        return action == 106;
    }
    
    public static final boolean isActionLoad(final short action) {
        return action == 605;
    }
    
    public static final boolean isActionAttachLock(final short action) {
        return action == 161;
    }
    
    public static final boolean isActionChangeBuilding(final short action) {
        return action == 683 || action == 847;
    }
    
    public static final boolean isActionLockPick(final short action) {
        return action == 101;
    }
    
    public static final boolean isActionMeditate(final short action) {
        return action == 384;
    }
    
    public static final boolean isActionMilkOrShear(final short action) {
        return action == 345 || action == 646;
    }
    
    public static final boolean isActionMine(final short action) {
        return action == 145 || action == 147 || action == 146;
    }
    
    public static final boolean isActionMineFloor(final short action) {
        return action == 145 || action == 150 || action == 532 || action == 518;
    }
    
    public static final boolean isActionMineSurface(final short action) {
        return action == 145 || action == 518;
    }
    
    public static final boolean isActionPack(final short action) {
        return action == 154;
    }
    
    public static final boolean isActionPave(final short action) {
        return action == 155 || action == 191 || action == 628 || action == 627 || action == 576 || action == 695 || action == 694 || action == 757;
    }
    
    public static final boolean isActionPickSprout(final short action) {
        return action == 187;
    }
    
    public static final boolean isActionPickupPlanted(final short action) {
        return action == 685;
    }
    
    public static final boolean isActionPlaceMerchants(final short action) {
        return action == 85;
    }
    
    public static final boolean isActionPlaceNPCs(final short action) {
        return action == 85 || (action == 863 && Features.Feature.WAGONER.isEnabled());
    }
    
    public static final boolean isActionPlanBuilding(final short action) {
        return action == 56 || action == 57 || action == 58;
    }
    
    public static final boolean isActionPlant(final short action) {
        return action == 186;
    }
    
    public static final boolean isActionPlantCenter(final short action) {
        return action == 660;
    }
    
    public static final boolean isActionPrune(final short action) {
        return action == 373;
    }
    
    public static final boolean isActionPullPushTurn(final short action) {
        return action == 99 || action == 181 || action == 696 || action == 697 || action == 177 || action == 178 || action == 926;
    }
    
    public static final boolean isActionReinforce(final short action) {
        return action == 229;
    }
    
    public static final boolean isActionSacrifice(final short action) {
        return action == 142;
    }
    
    public static final boolean isActionSorcerySpell(final short action) {
        return action == 547;
    }
    
    public static final boolean isActionSow(final short action) {
        return action == 153;
    }
    
    public static final boolean isActionTake(final int action) {
        return action == 6 || action == 16 || action == 465 || action == 464 || action == 133;
    }
    
    public static final boolean isActionTame(final short action) {
        return action == 46 || action == 275 || action == 274;
    }
    
    public static final boolean isActionTerraform(final short action, final boolean onSurface) {
        return (onSurface && (action == 150 || action == 532)) || action == 144 || action == 362 || action == 369 || action == 533 || action == 865 || action == 37 || action == 462 || action == 927;
    }
    
    public static final boolean isActionTrim(final short action) {
        return action == 644;
    }
    
    public static final boolean isActionTunnel(final short action) {
        return action == 227;
    }
    
    public static final boolean isActionUnload(final short action) {
        return action == 234;
    }
    
    public static final boolean isActionManage(final short action) {
        return action == 664 || action == 364 || action == 663 || action == 668 || action == 665 || action == 687 || action == 669 || action == 688 || action == 667 || action == 863;
    }
    
    public static final boolean isActionBuildingPermission(final short action) {
        return isActionAttachLock(action) || isActionBuild(action) || isActionDestroy(action) || isActionDestroyFence(action) || isActionDrop(action) || isActionImproveOrRepair(action) || isActionLoad(action) || isActionTake(action) || isActionPickupPlanted(action) || isActionPlaceMerchants(action) || isActionPlanBuilding(action) || isActionUnload(action);
    }
    
    public static final boolean isActionDisembark(final short action) {
        return action == 333 || action == 708;
    }
    
    public static final boolean isActionAllowedOnVehicle(final short action) {
    ...
    public static final boolean isActionAllowedOnBoat(final short action) {
        return action == 532 || action == 150;
    }
    
    public static final boolean isActionAllowedThroughFloors(final short action) {
        return action == 1 || action == 671 || action == 672;
    }
    
    public static final boolean isDefaultTerraformingAction(final short action) {
        return action == 718;
    }
    
    public static final short getDefaultActionForTarget(final long actionItemId, final long target) {
        final int counterType = WurmId.getType(target);
        switch (counterType) {
            case 3: {
                ...
                final Optional<Item> actionItem = Items.getItemOptional(actionItemId);
                if (type == Tiles.Tile.TILE_ROCK.id) {
                    if (actionItem.isPresent()) {
                        final Item item = actionItem.get();
                ...
                if (actionItem.isPresent()) {
                    final Item actionTool = actionItem.get();
            ...
            case 2: {
                final Optional<Item> targetOptional = Items.getItemOptional(target);
                if (targetOptional.isPresent()) {
                    final Item targetItem = targetOptional.get();
                    final Optional<Item> actionItem2 = Items.getItemOptional(actionItemId);
                    if (targetItem.getTemplateId() == 385 && actionItem2.isPresent()) {
                        return 97;
                    }
                    if (targetItem.isBoat() && actionItem2.isPresent()) {
                        final Item tool = actionItem2.get();
