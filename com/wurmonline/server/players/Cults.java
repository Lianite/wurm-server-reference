...
public final class Cults implements TimeConstants, MiscConstants
{
    ...
    private static final String[] YES_NO_STRINGS;
    private static final Logger logger;
    
    public static final byte getPathFor(final int tilex, final int tiley, final int layer, final int performerHeight) {
        byte path = 0;
        if (layer >= 0) {
            final short[] steepness = Creature.getTileSteepness(tilex, tiley, layer >= 0);
            if (steepness[1] > 10) {
                return 0;
            }
        }
        final int[][] closeTiles = getCloseTiles(tilex, tiley, layer);
        final Map<Byte, Integer> dominatingType = new HashMap<Byte, Integer>();
        ...
        int flowerTiles = 0;
        int thornTiles = 0;
        for (final int[] lCloseTile : closeTiles) {
            for (int y = 0; y < closeTiles.length; ++y) {
                boolean detectedWater = false;
                final byte type = Tiles.decodeType(lCloseTile[y]);
                ...
                final short height = Tiles.decodeHeight(lCloseTile[y]);
                final int diff = performerHeight - height;
                if (height < -5.0f) {
                    detectedWater = true;
                }
                else if (diff < -100) {
                ...
                if (type == Tiles.Tile.TILE_GRASS.id || type == Tiles.Tile.TILE_REED.id) {
                    final GrassData.FlowerType f = GrassData.FlowerType.decodeTileData(Tiles.decodeData(lCloseTile[y]));
        ...
        if (layer < 0) {
            ...
            int maxCount = 0;
            for (final Map.Entry<Byte, Integer> e : dominatingType.entrySet()) {
            ...
            final int westTile = Server.surfaceMesh.getTile(Zones.safeTileX(tilex + 20), Zones.safeTileY(tiley));
            short height2 = Tiles.decodeHeight(westTile);
            int diff2 = performerHeight - height2;
            if (height2 < -5.0f) {
                farWater = true;
            }
            else if (diff2 < -400) {
            ...
            final int northTile = Server.surfaceMesh.getTile(Zones.safeTileX(tilex), Zones.safeTileY(tiley - 20));
            height2 = Tiles.decodeHeight(northTile);
            diff2 = performerHeight - height2;
            if (height2 < -5.0f) {
                farWater = true;
            }
            else if (diff2 < -400) {
            ...
            final int southTile = Server.surfaceMesh.getTile(Zones.safeTileX(tilex), Zones.safeTileY(tiley + 20));
            height2 = Tiles.decodeHeight(southTile);
            diff2 = performerHeight - height2;
            if (height2 < -5.0f) {
                farWater = true;
            }
            ...
            final int eastTile = Server.surfaceMesh.getTile(Zones.safeTileX(tilex - 20), Zones.safeTileY(tiley));
            height2 = Tiles.decodeHeight(eastTile);
            diff2 = performerHeight - height2;
            if (height2 < -5.0f) {
                farWater = true;
            }
    ...
    static final int[][] getCloseTiles(final int tilex, final int tiley, final int layer) {
        final int[][] toReturn = new int[5][5];
        for (int x = -2; x < 3; ++x) {
            for (int y = -2; y < 3; ++y) {
                try {
                    if (layer >= 0) {
    ...
    public static final boolean meditate(final Creature performer, final int layer, final Action aAct, final float counter, final Item rug) {
        ...
        final Cultist cultist = Cultist.getCultist(performer.getWurmId());
        ...
        if (counter == 1.0f) {
            ...
            if (meditation.skillCheck(difficulty, rug, 0.0, true, 1.0f + rug.getRarity() * 0.1f) < 0.0) {
        ...
        if (counter * 10.0f > time) {
            ...
            if (cultist != null && meditation.getKnowledge(0.0) >= 20.0) {
                final long timeSinceLast = System.currentTimeMillis() - cultist.getLastMeditated();
                ...
                try {
                    ...
                    Cults.logger.log(Level.WARNING, performer.getName() + iox.getMessage(), iox);
                }
            }
            if (meditation.skillCheck(95.0, rug, 0.0, !increaseSkill, 1.0f + rug.getRarity() * 0.1f) > 0.0) {
            ...
            if (path != 0) {
                ...
                if (cultist != null) {
                    ...
                    if (timeToNextLevel <= 0L) {
                        if (cultist.getLevel() * 10 - meditation.getKnowledge(0.0) < 30.0 || meditation.getKnowledge(0.0) > 90.0) {
                            mayIncreaseLevel = true;
                            difficulty2 = 1 + cultist.getLevel() * 10;
                        }
                        else {
                            difficulty2 = 1 + cultist.getLevel() * 3;
            ...
            if (path != 0) {
                ...
                if (mayIncreaseLevel) {
                    if (power > 0.0) {
                        if (cultist == null) {
                            final CultQuestion cq = new CultQuestion(performer, "Following a path", "Do you wish to embark on a journey?", timeToNextLevel, null, path, false, false);
                            cq.sendQuestion();
                        }
                        else if (path == cultist.getPath()) {
                            if (cultist.getLevel() < 6 || performer.isPaying()) {
                                final CultQuestion cq = new CultQuestion(performer, "Following a path", "Are you ready to proceed?", timeToNextLevel, cultist, path, false, false);
                                cq.sendQuestion();
                            }
                        }
                        else {
                        ...
                        performer.getCommunicator().sendNormalServerMessage("Power of skillroll is " + power + " for difficulty " + difficulty2 + ".");
                    }
                }
            }
        }
    ...
    static String getWaitForProgressMessage(final long timeToNextLevel) {
        String message;
    ...
    public static final String getMeditationString(final byte path, final byte level, final int tilex, final int tiley) {
    ...
    static String getMeditationStringForKnowledgePath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    static String getMeditationStringForInsanityPath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    static String getMeditationStringForHatePath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    static String getMeditationStringForPowerPath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    static String getMeditationStringForLovePath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    static String getMeditationStringForNoPath(final int id) {
        String medString = null;
        switch (id) {
            case 0: {
    ...
    public static final String getNameForLevel(final byte path, final byte level) {
        String name = null;
        switch (path) {
            case 2: {
    ...
    static String getNameForLevelForInsanityPath(final byte level) {
        String name = null;
        if (level > 11) {
    ...
    static String getNameForLevelForPowerPath(final byte level) {
        String name = null;
        if (level > 11) {
    ...
    static String getNameForLevelForKnowledgePath(final byte level) {
        String name = null;
        if (level > 11) {
    ...
    static String getNameForLevelForLovePath(final byte level) {
        String name = null;
        if (level > 11) {
            name = level + "th Deva";
        }
    ...
    static String getNameForLevelForHatePath(final byte level) {
        String name = null;
        if (level > 11) {
    ...
    public static final String getQuestionForLevel(final byte path, final byte level) {
    ...
    static String getQuestionForLevelForHatePath(final byte level) {
    ...
    static String getQuestionForLevelForKnowledgePath(final byte level) {
        String question = null;
        switch (level) {
            case 0: {
    ...
    static String getQuestionForLevelForPowerPath(final byte level) {
        String question = null;
        switch (level) {
            case 0: {
    ...
    static String getQuestionForLevelForInsanityPath(final byte level) {
        String question = null;
        switch (level) {
            case 0: {
    ...
    static String getQuestionForLevelForLovePath(final byte level) {
        String question = null;
        switch (level) {
            case 0: {
    ...
    public static final String[] getAnswerAlternativesForLevel(final byte path, final byte level) {
        String[] alts = null;
        switch (path) {
            case 4: {
    ...
    static String[] getAnswerAlternativesForLevelForInsanityPath(final byte level) {
        String[] alts = null;
        switch (level) {
            case 0: {
    ...
    static String[] getAnswerAlternativesForLevelForPowerPath(final byte level) {
        String[] alts = null;
        switch (level) {
            case 0: {
    ...
    static String[] getAnswerAlternativesForLevelForLovePath(final byte level) {
        String[] alts = null;
        switch (level) {
            case 0: {
    ...
    static String[] getAnswerAlternativesForLevelForHatePath(final byte level) {
        String[] alts = null;
        switch (level) {
            case 0: {
    ...
    static String[] getAnswerAlternativesForLevelForKnowledgePath(final byte level) {
        String[] alts = null;
        switch (level) {
            case 0: {
    ...
    public static final String getWrongAnswerStringForLevel(final byte path, final byte level) {
    ...
    static String getWrongAnswerStringForLevelForPowerPath(final byte level) {
        String wrongAnswer = null;
        switch (level) {
            case 0: {
    ...
    static String getWrongAnswerStringForLevelForInsanityPath(final byte level) {
        String wrongAnswer = null;
        switch (level) {
            case 0: {
    ...
    static String getWrongAnswerStringForLevelForLovePath(final byte level) {
        String wrongAnswer = null;
        switch (level) {
            case 0: {
    ...
    static String getWrongAnswerStringForLevelForHatePath(final byte level) {
        String wrongAnswer = null;
        switch (level) {
            case 0: {
    ...
    static String getWrongAnswerStringForLevelForKnowledgePath(final byte level) {
        String wrongAnswer = null;
        switch (level) {
            case 0: {
    ...
    public static final int getCorrectAnswerForNextLevel(final byte path, final byte level) {
        int answerForNextLevel = 0;
        switch (path) {
            case 5: {
    ...
    static int getCorrectAnswerForNextLevelForPowerPath(final byte level) {
        int answerForNextLevel = 0;
        switch (level) {
            case 0: {
    ...
    static int getCorrectAnswerForNextLevelForInsanityPath(final byte level) {
        int answerForNextLevel = 0;
        switch (level) {
            case 0: {
                answerForNextLevel = 1;
    ...
    static int getCorrectAnswerForNextLevelForLovePath(final byte level) {
        int answerForNextLevel = 0;
        switch (level) {
            case 0: {
                answerForNextLevel = 0;
    ...
    static int getCorrectAnswerForNextLevelForHatePath(final byte level) {
        int answerForNextLevel = 0;
        switch (level) {
            case 0: {
                answerForNextLevel = 3;
    ...
    static int getCorrectAnswerForNextLevelForKnowledgePath(final byte level) {
        int answerForNextLevel = 0;
        switch (level) {
            case 0: {
                answerForNextLevel = 3;
    ...
    public static final String getCorrectAnswerStringForNextLevel(final byte path, final byte level) {
        String answerForNextLevel = null;
        switch (path) {
            case 5: {
    ...
    static String getCorrectAnswerStringForNextLevelForKnowledgePath(final byte level) {
        String answerForNextLevel = null;
        switch (level) {
            case 0: {
    ...
    static String getCorrectAnswerStringForNextLevelForHatePath(final byte level) {
        String answerForNextLevel = null;
        switch (level) {
            case 0: {
                answerForNextLevel = "You think 'I am on to something'";
    ...
    static String getCorrectAnswerStringForNextLevelForLovePath(final byte level) {
        String answerForNextLevel = null;
        switch (level) {
            case 0: {
                answerForNextLevel = "You think 'Yes, that probably is it'";
    ...
    static String getCorrectAnswerStringForNextLevelForInsanityPath(final byte level) {
        String answerForNextLevel = null;
        switch (level) {
            case 0: {
    ...
    static String getCorrectAnswerStringForNextLevelForPowerPath(final byte level) {
        String answerForNextLevel = null;
        switch (level) {
            case 0: {
                answerForNextLevel = "You think 'I am on to something'";
    ...
    public static final String getPathNameFor(final byte path) {
        switch (path) {
            case 2: {
    ...
    static {
        YES_NO_STRINGS = new String[] { "Yes", "No" };
        logger = Logger.getLogger(Cults.class.getName());
    }
}
