...
public class Trade implements MiscConstants
{
    ...
    private boolean creatureOneSatisfied;
    private boolean creatureTwoSatisfied;
    ...
    private long shopDiff;
    private long tax;
    
    public Trade(final Creature aCreatureOne, final Creature aCreatureTwo) {
        this.creatureOneSatisfied = false;
        this.creatureTwoSatisfied = false;
        this.currentCounter = -1;
        this.moneyAdded = 0L;
        this.shopDiff = 0L;
        this.tax = 0L;
        (this.creatureOne = aCreatureOne).startTrading();
        (this.creatureTwo = aCreatureTwo).startTrading();
    ...
    public void setMoneyAdded(final long money) {
        this.moneyAdded = money;
    }
    
    public void addShopDiff(final long money) {
    ...
    public TradingWindow getTradingWindow(final long id) {
        if (id == 1L) {
    ...
    public void setSatisfied(final Creature creature, final boolean satisfied, final int id) {
    ...
    private boolean makeTrade() {
        ...
        if (this.creatureOneRequestWindow.hasInventorySpace() && this.creatureTwoRequestWindow.hasInventorySpace()) {
            final int reqOneWeight = this.creatureOneRequestWindow.getWeight();
            final int reqTwoWeight = this.creatureTwoRequestWindow.getWeight();
                ...
                if (shop != null && this.shopDiff != 0L) {
                    if (this.shopDiff > 0L) {
                        if (shop.getTax() > 0.0f && !shop.isPersonal() && citizenVillage != null && this.creatureOne.citizenVillage != citizenVillage && citizenVillage.plan != null) {
                            this.setTax((long)(this.shopDiff * shop.getTax()));
                            Trade.logger.log(Level.INFO, this.creatureOne.getName() + " and " + this.creatureTwo.getName() + " adding " + this.getTax() + " tax to " + citizenVillage.getName());
    ...
    public void end(final Creature creature, final boolean closed) {
        try {
            if (creature.equals(this.creatureOne)) {
                this.creatureTwo.getCommunicator().sendCloseTradeWindow();
                if (!closed) {
            ...
            Trade.logger.log(Level.WARNING, nsp.getMessage(), nsp);
        }
        this.creatureTwoOfferWindow.endTrade();
        this.creatureOneOfferWindow.endTrade();
    ...
    void setCreatureOneSatisfied(final boolean aCreatureOneSatisfied) {
    ...
    void setCreatureTwoSatisfied(final boolean aCreatureTwoSatisfied) {
    ...
    void setCurrentCounter(final int aCurrentCounter) {
    ...
    public void setTax(final long aTax) {
        this.tax = aTax;
    }
    ...
    static {
        logger = Logger.getLogger(Trade.class.getName());
    }
}
