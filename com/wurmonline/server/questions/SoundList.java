...
public class SoundList extends Question
{
    ...
    public SoundList(final Creature _responder, final String _title, final String _question) {
    ...
    public void loadSoundStrings() {
        final LinkedList<soundString> soundsList = new LinkedList<soundString>();
    ...
    public void answer(final Properties aAnswers) {
        ...
        if (playSound) {
            final int sel = this.getIntProp("sel");
            for (final soundString ss : this.soundStrings) {
                if (ss.getId() == sel) {
                    SoundPlayer.playSound(this.selected = ss.getSoundName(), this.getResponder(), 1.5f);
                    this.reshow();
                    return;
                }
            }
        }
        final boolean select = this.getBooleanProp("select");
        if (select && this.root != null) {
            final int sel2 = this.getIntProp("sel");
            if (sel2 == 0) {
                this.root.cloneAndSendManageEffect(null);
                return;
            }
            for (final soundString ss2 : this.soundStrings) {
                if (ss2.getId() == sel2) {
                    this.root.cloneAndSendManageEffect(ss2.getSoundName());
                    return;
                }
            }
        }
    ...
    void reshow() {
        final SoundList sl = new SoundList(this.getResponder(), this.getTitle(), this.getQuestion());
        sl.root = this.root;
    ...
    public void sendQuestion() {
        ...
        switch (absSortBy) {
            case 1: {
                Arrays.sort(this.soundStrings, new Comparator<soundString>() {
                    @Override
                    public int compare(final soundString param1, final soundString param2) {
        ...
        for (final soundString ss : this.soundStrings) {
            ...
            final boolean sel = this.selected.equals(ss.getSoundName());
            if (sel) {
                noneSel = false;
                show = true;
            }
    ...
    void setSelected(final String soundName) {
    ...
    class soundString
    {
        ...
        soundString(final String category, final String name, final String soundName) {
