...
public final class ServerQuestion extends Question implements CounterTypes
{
    ...
    public ServerQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final long aTarget) {
        super(aResponder, aTitle, aQuestion, 43, aTarget);
        this.serverEntries = new LinkedList<ServerEntry>();
        this.transferEntries = new LinkedList<ServerEntry>();
    }
    ...
    public void sendQuestion() {
        ...
        if (this.getResponder().getPower() > 0) {
            if (WurmId.getType(this.target) == 0) {
                try {
                    final Player p = Players.getInstance().getPlayer(this.target);
    ...
    private String getServerEntryData(final String serverType, final ServerEntry server) {
        final StringBuilder buf = new StringBuilder();
    ...
    ServerEntry getServerEntry(final int aPosition) {
        return this.serverEntries.get(aPosition);
    }
    
    ServerEntry getTransferEntry(final int aPosition) {
