...
public class FeatureManagement extends Question implements TimeConstants
{
    ...
    public FeatureManagement(final Creature aResponder, final long aTarget) {
    ...
    public void answer(final Properties aAnswer) {
        ...
        if (this.type == 0) {
            FeatureManagement.logger.log(Level.INFO, "Received answer for a question with NOQUESTION.");
            return;
        }
        if (this.type == 102 && this.getResponder().getPower() >= 4) {
            final Features.Feature[] features = Features.Feature.values();
            for (int x = 0; x < features.length; ++x) {
                if (features[x].isShown()) {
                    final boolean enabled = aAnswer.getProperty("enable" + x).equals("true");
                    final boolean override = aAnswer.getProperty("override" + x).equals("true");
                    final boolean global = aAnswer.getProperty("global" + x).equals("true");
                    if (global || features[x].isEnabled() != enabled || features[x].isOverridden() != override) {
                        this.somethingChanged = true;
                    }
                    final int featureId = features[x].getFeatureId();
                    Features.Feature.setOverridden(Servers.getLocalServerId(), featureId, override, enabled, global);
                }
            }
            if (this.somethingChanged) {
                final FeatureManagement fm = new FeatureManagement(this.getResponder(), this.target);
                fm.sendQuestion();
            }
        }
    }
    ...
    static {
        logger = Logger.getLogger(FeatureManagement.class.getName());
    }
}
