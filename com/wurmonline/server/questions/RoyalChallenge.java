...
public class RoyalChallenge extends Question implements TimeConstants
{
    ...
    public RoyalChallenge(final Creature aResponder) {
    ...
    public void answer(final Properties aAnswers) {
        if (this.getResponder().isKing()) {
            ...
            if (k != null) {
                ...
                final String key2 = "decide";
                final String val2 = aAnswers.getProperty("decide");
                ...
                if (decline) {
                    ...
                    if (k.hasFailedAllChallenges()) {
                        ...
                        RoyalChallenge.logger.log(Level.INFO, this.getResponder().getName() + " may now be voted away.");
                    }
                    return;
                }
                if (!accept) {
                    this.getResponder().getCommunicator().sendNormalServerMessage("You decide to wait with answering the challenge.");
                    final long timeLeft = 604800000L + k.getChallengeDate() - System.currentTimeMillis();
                    final String tl = Server.getTimeFor(timeLeft);
                ...
                final String valday = aAnswers.getProperty("day");
                if (valday != null && valday.length() > 0) {
                    try {
                        final int day = Integer.parseInt(valday);
                        final String keyhour = "hours";
                        final String valhour = aAnswers.getProperty("hours");
                        if (valhour != null && valhour.length() > 0) {
                            try {
                                final int hour = Integer.parseInt(valhour);
                                long time = System.currentTimeMillis() + day * 86400000L + hour * 3600000L;
                                ...
                                final Message mess = new Message(this.getResponder(), (byte)10, Kingdoms.getChatNameFor(this.getResponder().getKingdomId()), "<" + this.getResponder().getName() + "> has accepted the challenge and must be at the duelling ring exactly in " + day + " days and " + hour + " hours");
                                final Player[] playarr = Players.getInstance().getPlayers();
                                final byte windowKingdom = this.getResponder().getKingdomId();
            ...
            if (k != null) {
                ...
                if (k.hasFailedToRespondToChallenge()) {
                    ...
                    if (k.getChallengeAcceptedDate() > System.currentTimeMillis()) {
                        final long timeLeft = k.getChallengeAcceptedDate() - System.currentTimeMillis();
                        final String tl = Server.getTimeFor(timeLeft);
    ...
    static {
        RoyalChallenge.logger = Logger.getLogger(RoyalChallenge.class.getName());
    }
}
