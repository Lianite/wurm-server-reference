...
public final class TileDataQuestion extends Question
{
    ...
    public TileDataQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final int aTilex, final int aTiley, final long aTarget) {
    ...
    public void sendQuestion() {
        ...
        final int surfResource = Server.getWorldResource(this.tilex, this.tiley);
        final int surfMesh = Server.surfaceMesh.getTile(this.tilex, this.tiley);
        final byte surfData = Tiles.decodeData(surfMesh);
        final byte surfType = Tiles.decodeType(surfMesh);
        final short surfHeight = Tiles.decodeHeight(surfMesh);
        final Tiles.Tile surfTile = Tiles.getTile(surfType);
        final byte surfClientFlags = Server.getClientSurfaceFlags(this.tilex, this.tiley);
        final byte surfServerFlags = Server.getServerSurfaceFlags(this.tilex, this.tiley);
        final int caveResource = Server.getCaveResource(this.tilex, this.tiley);
        final int caveMesh = Server.caveMesh.getTile(this.tilex, this.tiley);
        final byte caveData = Tiles.decodeData(caveMesh);
        final byte caveType = Tiles.decodeType(caveMesh);
        final short caveHeight = Tiles.decodeHeight(caveMesh);
        final Tiles.Tile caveTile = Tiles.getTile(caveType);
        final byte caveClientFlags = Server.getClientCaveFlags(this.tilex, this.tiley);
        final byte caveServerFlags = Server.getServerCaveFlags(this.tilex, this.tiley);
        final int rockMesh = Server.rockMesh.getTile(this.tilex, this.tiley);
        final short rockHeight = Tiles.decodeHeight(rockMesh);
        ...
        if (surfTile.isBush() || surfTile.isTree()) {
            ...
            txUnused = "";
            if (surfTile.isTree() && (surfData >>> 4 & 0xF) == 0xF) {
                noGUnused = "";
                canHaveGrubs = true;
            }
            if (surfTile.isBush() && (surfData >>> 4 & 0xE) == 0xE) {
                noGUnused = "";
                canHaveGrubs = true;
            }
            if ((surfType == Tiles.Tile.TILE_TREE_BIRCH.id || surfType == Tiles.Tile.TILE_ENCHANTED_TREE_BIRCH.id || surfType == Tiles.Tile.TILE_MYCELIUM_TREE_BIRCH.id) && (surfData >>> 4 & 0xE) == 0xE) {
                noGUnused = "";
                canHaveGrubs = true;
            }
        }
            ...
            final int qlCnt = surfResource >>> 8 & 0xFF;
            final int dugCnt = surfResource & 0xFF;
        ...
        if (surfTile.isGrass()) {
            ...
            buf.append("label{text=\" Growth:\"};input{id=\"growth\";maxchars=\"1\";text=\"" + (surfData >>> 6 & 0x3) + "\"};");
            buf.append("label{text=\" GrassType(not used):\"};input{id=\"grasstype\";maxchars=\"1\";text=\"" + (surfData >>> 4 & 0x3) + "\"};");
            ...
            buf.append("label{text=\" Age:\"};input{id=\"age\";maxchars=\"3\";text=\"" + (surfData >>> 4 & 0xF) + "\"};");
            buf.append("label{text=\" Fruit:\"};input{id=\"harvestable\";maxchars=\"1\";text=\"" + (surfData >>> 3 & 0x1) + "\"};");
            buf.append("label{text=\" Centre:\"};input{id=\"incentre\";maxchars=\"1\";text=\"" + (surfData >>> 2 & 0x1) + "\"};");
            buf.append("label{text=\" Grass Length:\"};input{id=\"growth\";maxchars=\"1\";text=\"" + (surfData & 0x3) + "\"};");
            buf.append("}");
        }
        else if (surfType == Tiles.Tile.TILE_FIELD.id || surfType == Tiles.Tile.TILE_FIELD2.id) {
            ...
            buf.append("label{text=\" Dir:\"};dropdown{id='dir';options=\"none,NW,NE,SE,SW\";default=\"" + (surfData & 0x7) + "\"}");
            buf.append("}");
