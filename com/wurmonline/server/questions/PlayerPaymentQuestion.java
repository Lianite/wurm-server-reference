...
public final class PlayerPaymentQuestion extends Question implements MonetaryConstants
{
    ...
    public PlayerPaymentQuestion(final Creature aResponder) {
    ...
    public void answer(final Properties answers) {
        ...
        if (((Player)this.getResponder()).getSaveFile().getPaymentExpire() <= 0L || this.getResponder().hasFlag(63)) {
            if (money < 20000L) {
                ...
                final boolean purchaseFirstTime = Boolean.parseBoolean(answers.getProperty("purchaseFirstTime"));
                final long referredBy = ((Player)this.getResponder()).getSaveFile().referrer;
            ...
            final String purchaseStr = answers.getProperty("purchase");
                ...
                if (money >= 100000L) {
                    try {
                        if (this.getResponder().chargeMoney(100000L)) {
                            ...
                            lsw2.addPlayingTime(this.getResponder(), this.getResponder().getName(), 0, 30, System.currentTimeMillis() + Servers.localServer.name);
                            this.getResponder().getCommunicator().sendSafeServerMessage("Your request for playing time is being processed. It may take up to half an hour until the system is fully updated.");
                            Economy.getEconomy().getKingsShop().setMoney(Economy.getEconomy().getKingsShop().getMoney() + 30000L);
                            PlayerPaymentQuestion.logger.log(Level.INFO, this.getResponder().getName() + " purchased 1 month premium time for " + 10L + " silver coins. " + 30000L + " iron added to king.");
                        }
                        else {
                ...
                if (money >= 50000L) {
                    try {
                        if (this.getResponder().chargeMoney(50000L)) {
                            final LoginServerWebConnection lsw2 = new LoginServerWebConnection();
                            lsw2.addPlayingTime(this.getResponder(), this.getResponder().getName(), 0, 15, System.currentTimeMillis() + Servers.localServer.name);
                            this.getResponder().getCommunicator().sendSafeServerMessage("Your request for playing time is being processed. It may take up to half an hour until the system is fully updated.");
                            Economy.getEconomy().getKingsShop().setMoney(Economy.getEconomy().getKingsShop().getMoney() - 20000L);
                            PlayerPaymentQuestion.logger.log(Level.INFO, this.getResponder().getName() + " purchased 1 month premium time for " + 5L + " silver coins. " + -20000L + " iron added to king.");
                        }
                        else {
                            this.getResponder().getCommunicator().sendAlertServerMessage("Failed to charge you 10 silvers. Please try later.");
                        }
