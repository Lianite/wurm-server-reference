...
public final class ShowHarvestableInfo extends Question
{
    ...
    public ShowHarvestableInfo(final Creature aResponder, final Item paper, final WurmHarvestables.Harvestable harvestable) {
    ...
    private static String getReportName(final Item paper, final WurmHarvestables.Harvestable harvestable) {
        return LoginHandler.raiseFirstLetter(harvestable.getName() + " report (" + ShowHarvestableInfo.df.format(paper.getCurrentQualityLevel()) + ")");
    }
    
    public ShowHarvestableInfo(final Creature aResponder, final Item almanac) {
        this(aResponder, almanac, 0);
    }
    
    public ShowHarvestableInfo(final Creature aResponder, final Item almanac, final int summarytype) {
    ...
    private static String getSummaryName(final Item almanac, final int summarytype) {
        ...
        final String almanacType = (almanac.getTemplateId() == 1127) ? "Almanac" : almanac.getName();
    ...
    public void answer(final Properties answers) {
        ...
        if (this.type == 0) {
            ShowHarvestableInfo.logger.log(Level.INFO, "Received answer for a question with NOQUESTION.");
            return;
        }
        if (this.type == 141 && this.almanac != null) {
            final boolean show = this.getBooleanProp("show");
            final String sbyid = this.getAnswer().getProperty("by");
            final int byid = Integer.parseInt(sbyid);
            if (show) {
                final ShowHarvestableInfo shi = new ShowHarvestableInfo(this.getResponder(), this.almanac, byid);
                shi.sendQuestion();
            }
        }
    }
    ...
    private String showSummary() {
        final StringBuilder buf = new StringBuilder();
        buf.append("label{text=\"\"}");
        final Item[] reports = Methods.getBestReports(this.getResponder(), this.almanac);
        if (reports.length == 0) {
            ...
            for (final Item report : reports) {
                ...
                if (harvestable != null) {
                    ...
                    final long howGood = (long)(report.getCurrentQualityLevel() * 86400.0f);
                    String harvestStart = "";
                    String harvestEnd = "";
                    String colourStart = "";
                    String colourState = "";
                    ...
                    if (output) {
                        buf.append("label{" + colourState + "text=\"" + harvestable.getName() + "\"};label{" + colourState + "text=\"" + ShowHarvestableInfo.df.format(report.getCurrentQualityLevel()) + "\"};label{" + colourState + "text=\"" + harvestable.getState() + "\"};label{" + colourStart + "text=\"" + harvestStart + "\"};label{" + colourState + "text=\"" + harvestEnd + "\"};");
                        showedSome = true;
                    }
                }
                else {
    ...
    private String showHarvestableTimes() {
        ...
        final long howGood = (long)(this.paper.getCurrentQualityLevel() * 86400.0f);
    ...
    private boolean knowHarvestEnd(final WurmHarvestables.Harvestable harvestable, final long howGood) {
        return harvestable.getSeasonEnd() - howGood < WurmCalendar.getCurrentTime();
    }
    
    private boolean knowHarvestStart(final WurmHarvestables.Harvestable harvestable, final long howGood) {
