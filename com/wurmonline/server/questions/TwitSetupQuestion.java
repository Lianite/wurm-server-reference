...
public class TwitSetupQuestion extends Question
{
    ...
    public TwitSetupQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final long aTarget, final boolean village) {
        ...
        if (this.isVillage) {
            try {
                ...
                if (_consumerKeyToUse == null || _consumerSecretToUse == null || _applicationToken == null || _applicationSecret == null) {
                    TwitSetupQuestion.logger.info(this.getResponder() + " has cleared the Twitter credentials for Settlement: " + village);
                    ...
                    if (village.canTwit()) {
                        TwitSetupQuestion.logger.info(this.getResponder() + " has set the Twitter credentials for Settlement: " + village);
                        this.getResponder().getCommunicator().sendNormalServerMessage("Allright, twit away!");
                    }
                    else {
                        TwitSetupQuestion.logger.info(this.getResponder() + " has set invalid Twitter credentials for Settlement: " + village + ", so Twitter is now disabled.");
            ...
            if (_consumerKeyToUse == null || _consumerSecretToUse == null || _applicationToken == null || _applicationSecret == null) {
                TwitSetupQuestion.logger.info(this.getResponder() + " has cleared the Twitter credentials for this server, whose ID is: " + Servers.localServer.id);
                if (champtwits) {
                ...
                if (Servers.localServer.canTwitChamps) {
                    TwitSetupQuestion.logger.info(this.getResponder() + " has set the Champion Twitter credentials for this server, whose ID is: " + Servers.localServer.id);
                    this.getResponder().getCommunicator().sendNormalServerMessage("Allright, twit away!");
                }
                else {
                    TwitSetupQuestion.logger.info(this.getResponder() + " has set invalid Champion Twitter credentials for this server, whose ID is: " + Servers.localServer.id + ", so Champion Twitter is now disabled.");
                    this.getResponder().getCommunicator().sendNormalServerMessage("You won't be twittin' with those keys.");
                }
            }
            else {
                ...
                if (Servers.localServer.canTwit()) {
                    TwitSetupQuestion.logger.info(this.getResponder() + " has set the Twitter credentials for this server, whose ID is: " + Servers.localServer.id);
                    this.getResponder().getCommunicator().sendNormalServerMessage("Allright, twit away!");
                }
                else {
                    TwitSetupQuestion.logger.info(this.getResponder() + " has set invalid Twitter credentials for this server, whose ID is: " + Servers.localServer.id + ", so Twitter is now disabled.");
                    this.getResponder().getCommunicator().sendNormalServerMessage("You won't be twittin' with those keys.");
                }
            }
        }
        else {
            TwitSetupQuestion.logger.info(this.getResponder() + " tried to set the Twitter credentials but was not allowed.");
    ...
    static {
        TwitSetupQuestion.logger = Logger.getLogger(TwitSetupQuestion.class.getName());
    }
}
