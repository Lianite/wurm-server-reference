...
public final class SuicideQuestion extends Question
{
    ...
    public SuicideQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final long aTarget) {
        super(aResponder, aTitle, aQuestion, 48, aTarget);
    }
    
    @Override
    public void answer(final Properties answers) {
        final String key = "suicide";
        final String val = answers.getProperty("suicide");
        if (val != null && val.equals("true")) {
            ...
            if (this.getResponder().isTeleporting()) {
                ...
                SuicideQuestion.logger.log(Level.INFO, this.getResponder().getName() + " SUICIDE " + this.getResponder().getName() + " at coords: " + this.getResponder().getTileX() + ", " + this.getResponder().getTileY());
    ...
    public void sendQuestion() {
        ...
        try {
            final Skill body = this.getResponder().getSkills().getSkill(102);
            if (this.getResponder().isPlayer() && !this.getResponder().isPaying() && body.getKnowledge() < 20.0 && body.minimum - body.getKnowledge() > 0.05000000074505806 && body.minimum - body.getKnowledge() <= 0.06) {
                lastDeath = true;
            }
        }
        catch (NoSuchSkillException nss) {
            this.getResponder().getSkills().learn(102, 1.0f);
            SuicideQuestion.logger.log(Level.WARNING, this.getResponder().getName() + " learnt body strength.");
        }
        if (lastDeath) {
    ...
    static {
        logger = Logger.getLogger(SuicideQuestion.class.getName());
    }
}
