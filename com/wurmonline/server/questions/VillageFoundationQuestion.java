...
public final class VillageFoundationQuestion extends Question implements VillageStatus, ItemTypes, MonetaryConstants, CounterTypes
{
    ...
    public VillageFoundationQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final long aTarget) {
    ...
    public VillageFoundationQuestion(final Creature aResponder, final String aTitle, final String aQuestion, final long aTarget, final Village aVillage, final boolean aExpanding) {
        super(aResponder, aTitle, aQuestion, 7, aTarget);
        this.expanding = false;
        this.error = "";
        this.sequence = 0;
    ...
    private boolean answersFail() {
        ...
        final String val = this.getAnswer().getProperty("cancel");
        ...
        if (this.expanding) {
            try {
                final Village oldvill = Villages.getVillage(this.deed.getData2());
    ...
    private boolean checkTile() {
        final VolaTile tile = Zones.getOrCreateTile(this.tokenx, this.tokeny, this.surfaced);
        if (tile != null) {
    ...
    private void createIntro() {
        final VillageFoundationQuestion vf = new VillageFoundationQuestion(this.getResponder(), "Settlement Application Form", "Welcome to the Settlement Application Form", this.target);
        if (vf != null) {
            this.copyValues(vf);
            vf.sequence = 0;
            vf.sendIntro();
    ...
    private void createQuestion1() {
        final VillageFoundationQuestion vf = new VillageFoundationQuestion(this.getResponder(), "Settlement Size", "Stage One - The size of your settlement", this.target);
        if (vf != null) {
            this.copyValues(vf);
            vf.sequence = 1;
            vf.sendQuestion();
    ...
    private void createQuestion2() {
        final VillageFoundationQuestion vfq = new VillageFoundationQuestion(this.getResponder(), "The Deed Perimeter", "Congratulations! You have chosen a deed that will be " + this.diameterX + " tiles by " + this.diameterY + " tiles.", this.target);
        if (vfq != null) {
            this.copyValues(vfq);
            vfq.sequence = 2;
            vfq.sendQuestion2();
    ...
    private void createQuestion3() {
        final VillageFoundationQuestion vfq = new VillageFoundationQuestion(this.getResponder(), "Naming Your Deed", "Your survey was a success!", this.target);
        if (vfq != null) {
            this.copyValues(vfq);
            vfq.sequence = 3;
            vfq.sendQuestion3();
    ...
    private void createQuestion4() {
        final VillageFoundationQuestion vfq = new VillageFoundationQuestion(this.getResponder(), "Guards", "Congratulations! You have reserved the name '" + this.villageName + "'", this.target);
        if (vfq != null) {
            this.copyValues(vfq);
            vfq.sequence = 4;
            vfq.sendQuestion4();
    ...
    private void createQuestion5() {
        final VillageFoundationQuestion vfq = new VillageFoundationQuestion(this.getResponder(), "Check your settings", "Check your settings and Found your settlement!", this.target);
        if (vfq != null) {
            this.copyValues(vfq);
            vfq.sequence = 5;
            vfq.sendQuestion5();
    ...
    private void createQuestion6() {
        if (!this.expanding) {
            ...
            final VillageFoundationQuestion vfq = new VillageFoundationQuestion(this.getResponder(), "CONGRATULATIONS!", "CONGRATULATIONS!", this.target);
            if (vfq != null) {
                this.copyValues(vfq);
                vfq.sequence = 6;
                vfq.sendQuestion6();
    ...
    boolean parseVillageFoundationQuestion1() {
        ...
        this.diameterX = this.selectedWest + this.selectedEast + 1;
        this.diameterY = this.selectedNorth + this.selectedSouth + 1;
        if (this.diameterX / this.diameterY > 4.0f || this.diameterY / this.diameterX > 4.0f) {
            this.error += "The deed would be too stretched. One edge is not allowed to be more than 4 times the length of the other.";
        }
        if (this.error.length() < 1) {
            final int xa = Zones.safeTileX(this.tokenx - this.selectedWest);
            final int xe = Zones.safeTileX(this.tokenx + this.selectedEast);
            final int ya = Zones.safeTileY(this.tokeny - this.selectedNorth);
            final int ye = Zones.safeTileY(this.tokeny + this.selectedSouth);
            for (int x = xa; x <= xe; ++x) {
                for (int y = ya; y <= ye; ++y) {
                    ...
                    if (create) {
                        try {
                            final Item i = ItemFactory.createItem(671, 80.0f, this.getResponder().getName());
                            i.setPosXYZ((x << 2) + 2, (y << 2) + 2, Zones.calculateHeight((x << 2) + 2, (y << 2) + 2, true) + 5.0f);
                            Zones.getZone(x, y, true).addItem(i);
                        }
                        catch (Exception ex) {
                            VillageFoundationQuestion.logger.log(Level.INFO, ex.getMessage());
                        }
                    }
                }
            }
            ...
            for (int y = ya; y <= ye; ++y) {
                ...
                if (remove) {
                    try {
                        ...
                        VillageFoundationQuestion.logger.log(Level.INFO, nsz.getMessage());
                        notFound = true;
                        break;
                    }
                }
    ...
    private void removePerimeterMarkers() {
        final int xa = Zones.safeTileX(this.tokenx - this.selectedWest - 5 - this.initialPerimeter);
        final int xe = Zones.safeTileX(this.tokenx + this.selectedEast + 5 + this.initialPerimeter);
        final int ya = Zones.safeTileY(this.tokeny - this.selectedNorth - 5 - this.initialPerimeter);
        final int ye = Zones.safeTileY(this.tokeny + this.selectedSouth + 5 + this.initialPerimeter);
        boolean notFound = false;
        for (int x = xa; x <= xe && !notFound; ++x) {
            for (int y = ya; y <= ye; ++y) {
                boolean remove = false;
                ...
                if (remove) {
                    try {
                        final Zone zone = Zones.getZone(x, y, this.surfaced);
            ...
            try {
                ...
                for (int x = xa; x <= xe; ++x) {
                    for (int y = ya; y <= ye; ++y) {
                        ...
                        if (create) {
                            try {
                                final Item i = ItemFactory.createItem(673, 80.0f, this.getResponder().getName());
                                i.setPosXYZ((x << 2) + 2, (y << 2) + 2, Zones.calculateHeight((x << 2) + 2, (y << 2) + 2, true) + 5.0f);
                                Zones.getZone(x, y, true).addItem(i);
                            }
                            catch (Exception ex) {
    ...
    boolean parseVillageFoundationQuestion3() {
        ...
        if (this.expanding) {
            try {
                final Village v = Villages.getVillage(this.deed.getData2());
                if (v.mayChangeName()) {
    ...
    private long getResizeCostDiff() {
        ...
        try {
            final Village oldvill = Villages.getVillage(this.deed.getData2());
            long moneyNeeded = 0L;
            final long moneyToRefund = 0L;
            final int diffDeed = this.tiles - oldvill.getNumTiles();
            final int diffPerim = this.perimeterTiles - oldvill.getPerimeterNonFreeTiles();
            final long costDeedDiff = diffDeed * Villages.TILE_COST;
            final long costPerimDiff = diffPerim * Villages.PERIMETER_COST;
            final long costTotalDiff = costDeedDiff + costPerimDiff;
            if (costTotalDiff > 0L) {
            ...
            final int diffGuard = this.selectedGuards - oldvill.plan.getNumHiredGuards();
            if (diffGuard > 0) {
                moneyNeeded += diffGuard * Villages.GUARD_COST;
            }
            moneyNeeded -= 0L;
            return moneyNeeded;
        }
    ...
    static long getExpandMoneyNeededFromBank(final long moneyNeeded, final Village village) {
        final long moneyAvailInPlan = village.getAvailablePlanMoney();
    ...
    private long getFoundingCost() {
        long moneyNeeded = this.tiles * Villages.TILE_COST;
        moneyNeeded += this.perimeterTiles * Villages.PERIMETER_COST;
        moneyNeeded += this.selectedGuards * Villages.GUARD_COST;
        return moneyNeeded;
    }
    ...
    boolean parseVillageFoundationQuestion5() {
        final String key = "back";
        final String val = this.getAnswer().getProperty("back");
        if (val != null && val.equals("true")) {
            this.createQuestion4();
            return false;
        }
        ...
        try {
            Label_1914: {
                if (!this.expanding) {
                    final long toCharge = this.getFoundingCharge();
                    final boolean charge = toCharge > 0L;
                        ...
                        Label_0901: {
                            try {
                                ...
                                if (this.getResponder().getPower() < 5 && this.deed.getTemplateId() != 862) {
                                    ...
                                    shop.setMoney(shop.getMoney() - (int)(this.deed.getValue() * 0.4f));
                                }
                                if (left > 0L) {
                                ...
                                VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to create settlement: " + nsp.getMessage(), nsp);
                                ...
                                VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to create settlement: " + nsc.getMessage(), nsc);
                                this.getResponder().getCommunicator().sendNormalServerMessage("Failed to locate a resource needed for that request. Please contact administration.");
                                return false;
                            }
                            catch (IOException iox) {
                                VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to create settlement:" + iox.getMessage(), iox);
                                this.getResponder().getCommunicator().sendNormalServerMessage("Failed to locate a resource needed for that request. Please contact administration.");
                                return false;
                            }
                            catch (NoSuchRoleException nsr) {
                                VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to create settlement:" + nsr.getMessage(), nsr);
                                ...
                                VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to create settlement:" + nsi.getMessage(), nsi);
                    ...
                    final Change funds = new Change(toCharge);
                ...
                if (!this.permanent && moneyNeeded > 0L) {
                    final long rest = getExpandMoneyNeededFromBank(moneyNeeded, oldvill);
                    if (rest > 0L) {
                    ...
                    if (!lsw.addMoney(this.getResponder().getWurmId(), this.getResponder().getName(), Math.abs(moneyNeeded), "Resize " + oldvill.getName())) {
                        change = false;
                        VillageFoundationQuestion.logger.log(Level.INFO, "Changing values did not yield money for " + oldvill.getName() + " to " + this.getResponder().getName() + ": " + Math.abs(moneyNeeded) + "?");
            ...
            VillageFoundationQuestion.logger.log(Level.INFO, "Failed to create settlement:" + iox2.getMessage(), iox2);
    ...
    private boolean checkSize() {
        if (this.getResponder().getPower() < 3) {
            if (Features.Feature.TOWER_CHAINING.isEnabled()) {
                final InfluenceChain chain = InfluenceChain.getInfluenceChain(this.getResponder().getKingdomId());
                boolean found = false;
                for (final Item marker : chain.getChainMarkers()) {
                    if (!marker.isChained() && this.getResponder().getKingdomId() != 4) {
                        continue;
                    }
                    if (!marker.isGuardTower()) {
                        continue;
                    }
                    if (Math.abs(this.tokenx - marker.getTileX()) <= 50 && Math.abs(this.tokeny - marker.getTileY()) <= 50) {
                        found = true;
                        break;
                    }
                }
        ...
        final Map<Village, String> decliners = Villages.canFoundVillage(this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, this.initialPerimeter, true, oldvill, this.getResponder());
        ...
        if (checkFocusZones) {
            final String focusZoneReject = Villages.isFocusZoneBlocking(this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, this.initialPerimeter, true);
    ...
    public boolean checkDeedItem() {
        try {
            ...
            VillageFoundationQuestion.logger.log(Level.WARNING, "Failed to locate settlement deed with id " + this.target, nsi);
    ...
    private boolean checkBlockingCreatures() {
        ...
        final Object cret = Villages.isAggOnDeed(currVill, this.getResponder(), this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, true);
        if (cret != null) {
    ...
    private boolean checkBlockingKingdoms() {
        if (this.getResponder().getPower() < 2) {
            ...
            int eZoneEndX = -1;
            int eZoneEndY = -1;
            final Village existing = Villages.getVillage(this.tokenx, this.tokeny, true);
            if (existing != null) {
    ...
    private boolean checkBlockingItems() {
        if (this.getResponder().getPower() < 2) {
            final int maxnorth = Math.max(0, this.tokeny - this.selectedNorth - (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
            final int maxsouth = Math.min(Zones.worldTileSizeY, this.tokeny + this.selectedSouth + (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
            final int maxwest = Math.max(0, this.tokenx - this.selectedWest - (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
            final int maxeast = Math.min(Zones.worldTileSizeX, this.tokenx + this.selectedEast + (Servers.localServer.isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
            final int maxcnorth = Math.max(0, this.tokeny - this.selectedNorth - (Servers.localServer.isChallengeServer() ? 20 : 60));
            final int maxcsouth = Math.min(Zones.worldTileSizeY, this.tokeny + this.selectedSouth + (Servers.localServer.isChallengeServer() ? 20 : 60));
            final int maxcwest = Math.max(0, this.tokenx - this.selectedWest - (Servers.localServer.isChallengeServer() ? 20 : 60));
            final int maxceast = Math.min(Zones.worldTileSizeX, this.tokenx + this.selectedEast + (Servers.localServer.isChallengeServer() ? 20 : 60));
            final Rectangle2D rectangleToCheck = new Rectangle2D.Float(maxwest, maxnorth, maxeast - maxwest, maxsouth - maxnorth);
            ...
            for (final Item targ : Items.getSupplyDepots()) {
                if (targ.getTileX() > maxcwest && targ.getTileX() < maxceast && targ.getTileY() < maxcsouth && targ.getTileY() > maxcnorth) {
                    this.getResponder().getCommunicator().sendSafeServerMessage("You cannot found the settlement here, since this is a battle ground.");
                    return false;
                }
            }
            final Item altar = Villages.isAltarOnDeed(this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, true);
            if (altar != null) {
    ...
    private boolean checkStructuresInArea() {
        if (this.getResponder().getPower() <= 1) {
            final ArrayList<Structure> newStructures = new ArrayList<Structure>();
            final ArrayList<Structure> oldStructures = new ArrayList<Structure>();
            final Structure[] surfStructures = Zones.getStructuresInArea(this.tokenx - this.selectedWest - 2, this.tokeny - this.selectedNorth - 2, this.tokenx + this.selectedEast + 2, this.tokeny + this.selectedSouth + 2, true);
            final Structure[] caveStructures = Zones.getStructuresInArea(this.tokenx - this.selectedWest - 2, this.tokeny - this.selectedNorth - 2, this.tokenx + this.selectedEast + 2, this.tokeny + this.selectedSouth + 2, false);
            for (final Structure c : surfStructures) {
                newStructures.add(c);
            }
            for (final Structure c : caveStructures) {
                newStructures.add(c);
            }
            if (this.expanding) {
                try {
                    final Village oldvill = Villages.getVillage(this.deed.getData2());
                    final Structure[] surfStructuresOld = Zones.getStructuresInArea(oldvill.getStartX(), oldvill.getStartY(), oldvill.getEndX(), oldvill.getEndY(), true);
                    final Structure[] caveStructuresOld = Zones.getStructuresInArea(oldvill.getStartX(), oldvill.getStartY(), oldvill.getEndX(), oldvill.getEndY(), true);
                    for (final Structure c2 : surfStructuresOld) {
                        oldStructures.add(c2);
                    }
                    for (final Structure c2 : caveStructuresOld) {
                        oldStructures.add(c2);
                    }
                }
            ...
            if (newStructures.size() > 0) {
                boolean ok = false;
                for (final Structure lStructure : newStructures) {
                    ok = false;
                    if (this.expanding) {
                        for (final Structure lOldstructure : oldStructures) {
    ...
    private void addTileCost(final StringBuilder buf) {
        buf.append("text{text=\"You selected a size of " + this.diameterX + " by " + this.diameterY + ".\"}");
        if (!this.expanding) {
            if (!Servers.localServer.isFreeDeeds()) {
                buf.append("text{text=\"The Purchase price for these tiles are " + new Change(this.tiles * Villages.TILE_COST).getChangeString() + ".\"}");
            }
            if (Servers.localServer.isUpkeep()) {
                buf.append("text{text=\"The Monthly upkeep is " + new Change(this.tiles * Villages.TILE_UPKEEP).getChangeString() + ".\"}");
            }
            ...
            try {
                final Village oldvill = Villages.getVillage(this.deed.getData2());
                final int diff = this.tiles - oldvill.getNumTiles();
                if (diff > 0 && !Servers.localServer.isFreeDeeds()) {
                    buf.append("text{text=\"The initial cost for the tiles will be " + new Change(diff * Villages.TILE_COST).getChangeString() + ".\"}");
                }
                if (Servers.localServer.isUpkeep()) {
                    buf.append("text{text=\"The new monthly upkeep cost for the tiles will be " + new Change(this.tiles * Villages.TILE_UPKEEP).getChangeString() + ".\"}");
                }
                buf.append("text{text=\"\"}");
            }
            catch (NoSuchVillageException nsv) {
    ...
    private void addPerimeterCost(final StringBuilder buf) {
        if (!this.expanding) {
            ...
            if (this.initialPerimeter > 0) {
                if (!Servers.localServer.isFreeDeeds()) {
                    buf.append("text{text=\"The initial cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_COST).getChangeString() + ".\"}");
                }
                if (Servers.localServer.isUpkeep()) {
                    buf.append("text{text=\"The monthly upkeep cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_UPKEEP).getChangeString() + ".\"}");
                }
            }
            buf.append("text{text=\"\"}");
        }
            ...
            try {
                ...
                final int diff = this.perimeterTiles - oldvill.getPerimeterNonFreeTiles();
                if (diff > 0 && !Servers.localServer.isFreeDeeds()) {
                    buf.append("text{text=\"The additional cost for the extra perimeter tiles will be " + new Change(diff * Villages.PERIMETER_COST).getChangeString() + ".\"}");
                }
                if (Servers.localServer.isUpkeep()) {
                    if (this.initialPerimeter > 0) {
                        buf.append("text{text=\"The new monthly upkeep cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_UPKEEP).getChangeString() + ".\"}");
                    }
                    else if (diff < 0) {
    ...
    private void addCitizenMultiplier(final StringBuilder buf) {
        ...
        if (this.expanding) {
            try {
                final Village old = Villages.getVillage(this.deed.getData2());
                final Citizen[] citizens = old.getCitizens();
                int curr = 0;
                for (final Citizen lCitizen : citizens) {
                ...
                VillageFoundationQuestion.logger.log(Level.WARNING, nsv.getMessage());
            }
        }
    ...
    private void addTotalUpkeep(final StringBuilder buf) {
        if (Servers.localServer.isUpkeep()) {
            buf.append("text{text=\"Total upkeep per month will be " + new Change(this.totalUpkeep).getChangeString() + ".\"}");
        }
    }
    
    private void addGuardCost(final StringBuilder buf) {
        if (!this.expanding) {
            if (this.selectedGuards > 0) {
                if (Servers.localServer.isFreeDeeds()) {
                    ...
                    buf.append("text{text=\"You will hire " + this.selectedGuards + " guards for a cost of " + new Change(this.selectedGuards * Villages.GUARD_COST).getChangeString() + ".\"}");
                }
                if (Servers.localServer.isUpkeep()) {
            ...
            try {
                final Village oldvill = Villages.getVillage(this.deed.getData2());
                final int diff = this.selectedGuards - oldvill.plan.getNumHiredGuards();
                if (diff > 0) {
                    if (Servers.localServer.isFreeDeeds()) {
                        buf.append("text{text=\"You will hire " + diff + " new guards.\"}");
                    }
                    else {
                        buf.append("text{text=\"You will hire " + diff + " new guards for a cost of " + new Change(diff * Villages.GUARD_COST).getChangeString() + ".\"}");
                    }
                    if (Servers.localServer.isUpkeep()) {
    ...
    public void sendQuestion3() {
        ...
        if (this.expanding) {
            try {
                final Village village = Villages.getVillage(this.deed.getData2());
        ...
        final Kingdom k = Kingdoms.getKingdom(this.spawnKingdom);
    ...
    public void sendQuestion5() {
        ...
        if (!hasError) {
            ...
            buf.append("text{text=\"\"}");
            if (this.expanding) {
                final long resCost = this.getResizeCostDiff();
                if (resCost > 0L) {
                    final Change needed = new Change(resCost);
                    final long avail = village.getAvailablePlanMoney();
                    final Change availc = new Change(avail);
                    ...
                    final long rest = getExpandMoneyNeededFromBank(resCost, village);
                    if (rest > 0L) {
                        final Change restc = new Change(rest);
                ...
                final long fullcost = this.getFoundingCost();
                final Change cfull = new Change(fullcost);
                ...
                final long toCharge = this.getFoundingCharge();
                if (toCharge > 0L) {
                    final Change cc = new Change(toCharge);
                    ...
                    final long left = 100000L - fullcost;
                    final Change leftc = new Change(left);
    ...
    private void copyValues(final VillageFoundationQuestion vfq) {
        vfq.dir = this.dir;
    ...
    private boolean willHaveHighay() {
        final int startx = this.tokenx - this.selectedWest;
        final int starty = this.tokeny - this.selectedNorth;
        final int endx = this.tokenx + this.selectedEast;
        final int endy = this.tokeny + this.selectedSouth;
    ...
    public void setSequence(final int newseq) {
        this.sequence = newseq;
    }
    
    static {
        logger = Logger.getLogger(VillageFoundationQuestion.class.getName());
    }
}
