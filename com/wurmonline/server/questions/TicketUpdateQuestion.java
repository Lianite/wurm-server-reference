...
public class TicketUpdateQuestion extends Question
{
    ...
    public TicketUpdateQuestion(final Creature aResponder, final int aTicketId, final short aAction) {
        super(aResponder, "Ticket: #" + aTicketId, makeQuestion(aResponder, aAction), 108, aTicketId);
        this.ticketId = aTicketId;
        this.action = aAction;
    }
    
    static String makeQuestion(final Creature aResponder, final short aAction) {
        switch (aAction) {
            case 588: {
    ...
    public void answer(final Properties aAnswer) {
        ...
        if (this.type == 0) {
            TicketUpdateQuestion.logger.log(Level.INFO, "Received answer for a question with NOQUESTION.");
            return;
        }
        if (this.type == 108) {
            this.ticketId = Integer.parseInt(aAnswer.getProperty("tid"));
            this.action = Short.parseShort(aAnswer.getProperty("action"));
            final boolean append = Boolean.parseBoolean(aAnswer.getProperty("append"));
            final byte level = Byte.parseByte(aAnswer.getProperty("level"));
            ...
            final Ticket ticket = Tickets.getTicket(this.ticketId);
            ...
            switch (this.action) {
                ...
                case 597: {
                    ...
                    final byte service = Byte.parseByte(aAnswer.getProperty("service"));
                    final byte courteous = Byte.parseByte(aAnswer.getProperty("courteous"));
                    final byte knowledgeable = Byte.parseByte(aAnswer.getProperty("knowledgeable"));
                    final boolean general1 = Boolean.parseBoolean(aAnswer.getProperty("general1"));
                    final boolean general2 = Boolean.parseBoolean(aAnswer.getProperty("general2"));
                    final boolean general3 = Boolean.parseBoolean(aAnswer.getProperty("general3"));
                    final boolean general4 = Boolean.parseBoolean(aAnswer.getProperty("general4"));
                    final boolean general5 = Boolean.parseBoolean(aAnswer.getProperty("general5"));
                    final boolean general6 = Boolean.parseBoolean(aAnswer.getProperty("general6"));
                    final boolean general7 = Boolean.parseBoolean(aAnswer.getProperty("general7"));
                    final byte general8 = (byte)((general1 ? 1 : 0) + (general2 ? 2 : 0) + (general3 ? 4 : 0) + (general4 ? 8 : 0) + (general5 ? 16 : 0) + (general6 ? 32 : 0) + (general7 ? 64 : 0));
                    final boolean quality1 = Boolean.parseBoolean(aAnswer.getProperty("quality1"));
                    final boolean quality2 = Boolean.parseBoolean(aAnswer.getProperty("quality2"));
                    final boolean quality3 = Boolean.parseBoolean(aAnswer.getProperty("quality3"));
                    final boolean quality4 = Boolean.parseBoolean(aAnswer.getProperty("quality4"));
                    final boolean quality5 = Boolean.parseBoolean(aAnswer.getProperty("quality5"));
                    final boolean quality6 = Boolean.parseBoolean(aAnswer.getProperty("quality6"));
                    final byte quality7 = (byte)((quality1 ? 1 : 0) + (quality2 ? 2 : 0) + (quality3 ? 4 : 0) + (quality4 ? 8 : 0) + (quality5 ? 16 : 0) + (quality6 ? 32 : 0));
                    final boolean irked1 = Boolean.parseBoolean(aAnswer.getProperty("irked1"));
                    final boolean irked2 = Boolean.parseBoolean(aAnswer.getProperty("irked2"));
                    final boolean irked3 = Boolean.parseBoolean(aAnswer.getProperty("irked3"));
                    final boolean irked4 = Boolean.parseBoolean(aAnswer.getProperty("irked4"));
                    final boolean irked5 = Boolean.parseBoolean(aAnswer.getProperty("irked5"));
                    final boolean irked6 = Boolean.parseBoolean(aAnswer.getProperty("irked6"));
                    final byte irked7 = (byte)((irked1 ? 1 : 0) + (irked2 ? 2 : 0) + (irked3 ? 4 : 0) + (irked4 ? 8 : 0) + (irked5 ? 16 : 0) + (irked6 ? 32 : 0));
                    ticket.addNewTicketAction((byte)14, this.getResponder().getName(), note, level, service, courteous, knowledgeable, general8, quality7, irked7);
                    break;
                }
            }
        }
    ...
    public void sendQuestion() {
        ...
        final String[] descLines = ticket.getDescription().split("\\n");
        final int descLineCount = descLines.length;
        ...
        final TicketAction[] ticketActions = ticket.getTicketActions(player);
        if (ticketActions.length == 0) {
            ...
            for (final TicketAction ta : ticketActions) {
                ...
                final String note = ta.getNotePlus(player);
                if (note.length() > 0) {
        ...
        final int height = Math.min(220 + ticketActions.length * 23 + lines * 23 + descLineCount * 23, 500);
        this.getResponder().getCommunicator().sendBml(500, height, true, true, buf.toString(), 200, 200, 200, this.title);
    }
    
    private void showReadOnlyFeedback(final Ticket ticket, final StringBuilder buf) {
    ...
    static {
        logger = Logger.getLogger(TicketUpdateQuestion.class.getName());
    }
}
