...
public final class ProtocolUtilities
{
    ...
    public static String getDescriptionForCommand(final byte aCommandByte) {
        ...
        switch (aCommandByte) {
            ...
            default: {
                ProtocolUtilities.logger.warning("Unknown command byte: '" + Byte.toString(aCommandByte) + '\'');
    ...
    public static String getDescriptionForToggle(final int aToggleCode) {
        String lDescription = null;
        switch (aToggleCode) {
            case 0: {
            ...
            default: {
                ProtocolUtilities.logger.warning("Unknown Toggle code: '" + aToggleCode + '\'');
    ...
    public static boolean isValidCommandType(final byte aCommandByte) {
    ...
    public static boolean getBooleanFromSingleByte(final ByteBuffer aByteBuffer) {
        return aByteBuffer.get() != 0;
    }
    
    public static void putBooleanIntoSingleByte(final ByteBuffer aByteBuffer, final boolean aValue) {
        aByteBuffer.put((byte)(aValue ? 1 : 0));
    }
    
    public static int getIntFromSingleByte(final ByteBuffer aByteBuffer) {
        return aByteBuffer.get() & 0xFF;
    }
    
    public static void putIntIntoSingleByte(final ByteBuffer aByteBuffer, final int aValue) {
        aByteBuffer.put((byte)aValue);
    }
    
    public static float getFloat64FromSingleByte(final ByteBuffer aByteBuffer) {
        return (aByteBuffer.get() & 0xFF) / 64.0f;
    }
    
    public static float getFloat255FromSingleByte(final ByteBuffer aByteBuffer) {
        return (aByteBuffer.get() & 0xFF) / 255.0f;
    }
    
    public static String getString(final ByteBuffer aByteBuffer) {
        final byte[] lStringBytes = new byte[aByteBuffer.get() & 0xFF];
        ...
        try {
            ...
            ProtocolUtilities.logger.log(Level.WARNING, "Could not create a UTF-8 String", e);
            lString = "";
        }
        return lString;
    }
    
    public static void putString(final ByteBuffer aByteBuffer, final String aValue) {
        if (aValue.length() > 255) {
            ProtocolUtilities.logger.warning("Only the first 255 characters will be put in the ByteBuffer, String: " + aValue);
        }
        try {
            final byte[] lMessageBytes = aValue.getBytes("UTF-8");
            aByteBuffer.put((byte)lMessageBytes.length);
            aByteBuffer.put(lMessageBytes);
        }
        catch (UnsupportedEncodingException e) {
            ProtocolUtilities.logger.log(Level.WARNING, "Could not create a UTF-8 String so putting length 0 in the Buffer, String: " + aValue, e);
            aByteBuffer.put((byte)0);
        }
    }
    
    static {
        ProtocolUtilities.logger = Logger.getLogger(ProtocolUtilities.class.getName());
    }
}
