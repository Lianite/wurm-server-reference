...
public final class SkillSystem implements TimeConstants
{
    ...
    public static String getNameFor(final int skillNum) {
        final String name = SkillSystem.skillNames.get(skillNum);
        if (name != null) {
            return name;
        }
        return "unknown";
    ...
    public static float getNextRandomFloat() {
        return SkillSystem.randomSource.nextFloat();
    }
    
    @Nonnull
    public static int[] getDependenciesFor(final int skillNum) {
    ...
    static long getDecayTimeFor(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.getDecayTime();
    }
    
    public static short getTypeFor(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.getType();
    }
    
    public static long getTickTimeFor(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.getTickTime();
    }
    
    static boolean isFightingSkill(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.fightSkill;
    }
    
    static boolean isThieverySkill(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.thieverySkill;
    }
    
    private static void addSkillTemplate(final SkillTemplate template) {
        SkillSystem.templates.put(template.getNumber(), template);
        SkillStat.addSkill(template.getNumber(), template.name);
        SkillSystem.skillNames.put(template.getNumber(), template.getName());
        SkillSystem.namesToSkill.put(template.getName().toLowerCase(), template.getNumber());
        SkillSystem.skillnums = SkillSystem.templates.keySet().toArray(new Integer[SkillSystem.templates.size()]);
        SkillSystem.templateList.add(template);
    }
    
    static boolean ignoresEnemies(final int skillNum) {
        final SkillTemplate template = SkillSystem.templates.get(skillNum);
        return template.ignoresEnemies;
    }
    
    public static int getRandomSkillNum() {
        return SkillSystem.skillnums[Server.rand.nextInt(SkillSystem.templates.size())];
    }
    
    public static SkillTemplate[] getAllSkillTemplates() {
        return SkillSystem.templates.values().toArray(new SkillTemplate[SkillSystem.templates.size()]);
    }
    
    public static int getSkillByName(final String name) {
        final Integer i = SkillSystem.namesToSkill.get(name.toLowerCase());
        if (i == null) {
            return -1;
        }
        return i;
    ...
    public static SkillTemplate getSkillTemplateByIndex(final int index) {
        if (index > SkillSystem.templateList.size()) {
            return null;
        }
        return SkillSystem.templateList.get(index);
    }
    
    public static int getNumberOfSkillTemplates() {
        return SkillSystem.templateList.size();
    }
    
    static {
        templates = new HashMap<Integer, SkillTemplate>();
        skillNames = new HashMap<Integer, String>();
        namesToSkill = new HashMap<String, Integer>();
        templateList = new ArrayList<SkillTemplate>();
        randomSource = new Random();
        SkillSystem.skillnums = new Integer[0];
        ...
        final SkillTemplate bct = new SkillTemplate(104, "Body control", 150000.0f, new int[] { 1 }, 1209600000L, (short)0, false, true);
        addSkillTemplate(bct);
        final SkillTemplate bst = new SkillTemplate(103, "Body stamina", 200000.0f, new int[] { 1 }, 1209600000L, (short)0, false, true);
        addSkillTemplate(bst);
        final SkillTemplate bsr = new SkillTemplate(102, "Body strength", 200000.0f, new int[] { 1 }, 1209600000L, (short)0, false, true);
        addSkillTemplate(bsr);
        final SkillTemplate mlg = new SkillTemplate(100, "Mind logic", 200000.0f, new int[] { 2 }, 1209600000L, (short)0, false, true);
        addSkillTemplate(mlg);
        final SkillTemplate msp = new SkillTemplate(101, "Mind speed", 200000.0f, new int[] { 2 }, 1209600000L, (short)0, false, true);
