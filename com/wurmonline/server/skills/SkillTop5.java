...
public class SkillTop5
{
    ...
    public SkillTop5(final int aNumber, final int aPos, final long aOwner, final String aOwnerName, final double aValue) {
        this.number = aNumber;
        this.pos = aPos;
        this.owner = aOwner;
    ...
    public static SkillTop5[][] getAllSkillsTop5() throws Exception {
        final SkillTop5[][] skillsTop5 = new SkillTop5[SkillTop5.skills.length][5];
        Connection dbcon = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            dbcon = DbConnector.getPlayerDbCon();
            ps = dbcon.prepareStatement("SELECT S.NUMBER, S.OWNER, P.NAME, S.VALUE FROM WURMPLAYERS.SKILLS S JOIN WURMPLAYERS.PLAYERS P ON S.OWNER=P.WURMID WHERE S.NUMBER=? AND P.POWER=0 ORDER BY S.VALUE DESC LIMIT 5;");
            for (int i = 0; i < SkillTop5.skills.length; ++i) {
                ps.setInt(1, SkillTop5.skills[i]);
                rs = ps.executeQuery();
                for (int count = 0; rs.next() && count < 5; ++count) {
                    final SkillTop5 skill = new SkillTop5(rs.getInt("NUMBER"), count, rs.getLong("OWNER"), rs.getString("NAME"), rs.getDouble("VALUE"));
