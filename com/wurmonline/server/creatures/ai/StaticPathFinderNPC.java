...
public final class StaticPathFinderNPC extends PathFinder implements MiscConstants
{
    ...
    public StaticPathFinderNPC(final boolean ignoresWalls) {
    ...
    Path startAstar(final int _startX, final int _startY, final int _endX, final int _endY) throws NoPathException {
        ...
        try {
            ...
            if (StaticPathFinderNPC.logger.isLoggable(Level.FINEST)) {
                StaticPathFinderNPC.logger.finest(this.creature.getName() + " astared a path.");
            }
        }
    ...
    public Path rayCast(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean surf, final int areaSz) throws NoPathException {
        ...
        this.endX = Math.min(StaticPathFinderNPC.WORLD_SIZE - 1, endTileX);
        this.endY = Math.min(StaticPathFinderNPC.WORLD_SIZE - 1, endTileY);
        this.surfaced = surf;
        this.areaSize = areaSz;
        this.setMesh();
    ...
    void setMesh() {
        ...
        StaticPathFinderNPC.mesh = new StaticPathMeshNPC(this.startX, this.startY, this.endX, this.endY, this.surfaced, this.areaSize);
        this.current = StaticPathFinderNPC.mesh.getStart();
        this.start = StaticPathFinderNPC.mesh.getStart();
        this.finish = StaticPathFinderNPC.mesh.getFinish();
        if (StaticPathFinderNPC.logger.isLoggable(Level.FINEST)) {
            StaticPathFinderNPC.logger.finest("Start is " + this.start.toString() + ", finish " + this.finish.toString());
        }
    }
    ...
    Path rayCast(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean aSurfaced) throws NoPathException {
        ...
        this.stepsTaken = 0;
        StaticPathFinderNPC.pathList = new LinkedList<PathTile>();
        ...
        while (!this.current.equals(this.finish)) {
            ...
            StaticPathFinderNPC.pathList.add(this.current);
        }
        return new Path(StaticPathFinderNPC.pathList);
    }
    ...
    PathTile step() throws NoPathException {
        ...
        if (raycend && Math.abs(this.endX - x) <= 1 && Math.abs(this.endY - y) <= 1) {
            ...
            x = this.endX;
            y = this.endY;
            StaticPathFinderNPC.logger.log(Level.INFO, "This really shouldn't happen i guess, since it should have been detected already.");
        }
        else {
        ...
        if (!StaticPathFinderNPC.mesh.contains(x, y)) {
            throw new NoPathException("Path missed at " + x + ", " + y);
        }
        PathTile toReturn = null;
        try {
            toReturn = StaticPathFinderNPC.mesh.getPathTile(x, y);
            ...
            StaticPathFinderNPC.logger.log(Level.WARNING, "OUT OF BOUNDS AT RAYCAST: " + x + ", " + y + ": " + ai.getMessage(), ai);
            StaticPathFinderNPC.logger.log(Level.WARNING, "Mesh info: " + StaticPathFinderNPC.mesh.getBorderStartX() + ", " + StaticPathFinderNPC.mesh.getBorderStartY() + ", to " + StaticPathFinderNPC.mesh.getBorderEndX() + ", " + StaticPathFinderNPC.mesh.getBorderEndY());
            StaticPathFinderNPC.logger.log(Level.WARNING, "Size of meshx=" + StaticPathFinderNPC.mesh.getSizex() + ", meshy=" + StaticPathFinderNPC.mesh.getSizey());
            throw new NoPathException("Path missed at " + x + ", " + y);
        }
        if (this.stepsTaken > this.maxSteps) {
            if (StaticPathFinderNPC.logger.isLoggable(Level.FINEST)) {
                StaticPathFinderNPC.logger.finest("Raycaster stops searching after " + this.stepsTaken + " steps, suspecting it missed the target.");
            }
    ...
    Path astar(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean aSurfaced) throws NoPathException {
        ...
        if (this.finish == null) {
            if (this.creature != null) {
                StaticPathFinderNPC.logger.log(Level.WARNING, this.creature.getName() + " finish=null at " + endTileX + ", " + endTileY);
            }
            else {
                StaticPathFinderNPC.logger.log(Level.WARNING, "Finish=null at " + endTileX + ", " + endTileY);
            }
            return null;
        }
        if (this.start == null) {
            if (this.creature != null) {
                StaticPathFinderNPC.logger.log(Level.WARNING, this.creature.getName() + " start=null at " + startTileX + ", " + startTileY);
            }
            else {
                StaticPathFinderNPC.logger.log(Level.WARNING, "start=null at " + startTileX + ", " + startTileY);
            }
            return null;
        }
        this.start.setDistanceFromStart(this.start, 0.0f);
        StaticPathFinderNPC.pathList.add(this.start);
        int pass;
        int lState;
        ...
        if (lState == 1) {
            if (pass > 4000) {
                ...
                StaticPathFinderNPC.logger.log(Level.INFO, cname + " pathed from " + this.startX + ", " + this.startY + " to " + this.endX + ", " + this.endY + " and found path after " + pass + " steps.");
            }
            return this.setPath();
        }
        if (lState == 2) {
    ...
    private void setDebug(final boolean aDebug) {
        this.debug = aDebug;
        if (StaticPathFinderNPC.logger.isLoggable(Level.FINEST)) {
            StaticPathFinderNPC.logger.finest("Debug in pathfinding - " + aDebug);
        }
    }
    ...
    int step2() {
        ...
        float score = 0.0f;
        PathTile best = StaticPathFinderNPC.pathList.get(StaticPathFinderNPC.pathList.size() - 1);
        PathTile now = null;
        for (int i = 0; i < StaticPathFinderNPC.pathList.size(); ++i) {
            now = StaticPathFinderNPC.pathList.get(i);
        ...
        now = best;
        StaticPathFinderNPC.pathList.remove(now);
        now.setUsed();
        final PathTile[] next = StaticPathFinderNPC.mesh.getAdjacent(now);
        for (int j = 0; j < next.length; ++j) {
            if (next[j] != null && this.canPass(now, next[j])) {
                if (!StaticPathFinderNPC.pathList.contains(next[j]) && next[j].isNotUsed()) {
                    StaticPathFinderNPC.pathList.add(next[j]);
                }
        ...
        if (StaticPathFinderNPC.pathList.isEmpty()) {
            return 2;
        }
        return 0;
    }
    ...
    Path setPath() {
        ...
        StaticPathFinderNPC.pList = new LinkedList<PathTile>();
        PathTile lastCurrent = now;
        while (!finished) {
            StaticPathFinderNPC.pList.add(now);
            final PathTile next = this.findLowestDist(this.start, now);
            if (lastCurrent.equals(next)) {
                finished = true;
                StaticPathFinderNPC.logger.log(Level.WARNING, "Loop in heuristicastar.");
            }
            lastCurrent = now;
            now = next;
            if (now.equals(stop)) {
        ...
        final Iterator<PathTile> it = StaticPathFinderNPC.pList.iterator();
        while (it.hasNext()) {
