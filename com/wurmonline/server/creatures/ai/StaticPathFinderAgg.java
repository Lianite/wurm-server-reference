...
public final class StaticPathFinderAgg extends PathFinder implements MiscConstants
{
    ...
    public StaticPathFinderAgg(final boolean ignoresWalls) {
    ...
    Path startAstar(final int _startX, final int _startY, final int _endX, final int _endY) throws NoPathException {
        ...
        try {
            ...
            if (StaticPathFinderAgg.logger.isLoggable(Level.FINEST)) {
                StaticPathFinderAgg.logger.finest(this.creature.getName() + " astared a path.");
            }
        }
    ...
    public Path rayCast(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean surf, final int areaSz) throws NoPathException {
        ...
        this.endX = Math.min(StaticPathFinderAgg.WORLD_SIZE - 1, endTileX);
        this.endY = Math.min(StaticPathFinderAgg.WORLD_SIZE - 1, endTileY);
        this.surfaced = surf;
        this.areaSize = areaSz;
        this.setMesh();
    ...
    void setMesh() {
        ...
        StaticPathFinderAgg.mesh = new StaticPathMeshAgg(this.startX, this.startY, this.endX, this.endY, this.surfaced, this.areaSize);
        this.current = StaticPathFinderAgg.mesh.getStart();
        this.start = StaticPathFinderAgg.mesh.getStart();
        this.finish = StaticPathFinderAgg.mesh.getFinish();
        if (StaticPathFinderAgg.logger.isLoggable(Level.FINEST)) {
            StaticPathFinderAgg.logger.finest("Start is " + this.start.toString() + ", finish " + this.finish.toString());
        }
    }
    ...
    Path rayCast(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean aSurfaced) throws NoPathException {
        ...
        this.stepsTaken = 0;
        StaticPathFinderAgg.pathList = new LinkedList<PathTile>();
        ...
        while (!this.current.equals(this.finish)) {
            ...
            StaticPathFinderAgg.pathList.add(this.current);
        }
        return new Path(StaticPathFinderAgg.pathList);
    }
    ...
    PathTile step() throws NoPathException {
        ...
        if (raycend && Math.abs(this.endX - x) <= 1 && Math.abs(this.endY - y) <= 1) {
            ...
            x = this.endX;
            y = this.endY;
            StaticPathFinderAgg.logger.log(Level.INFO, "This really shouldn't happen i guess, since it should have been detected already.");
        }
        else {
        ...
        if (!StaticPathFinderAgg.mesh.contains(x, y)) {
            throw new NoPathException("Path missed at " + x + ", " + y);
        }
        PathTile toReturn = null;
        try {
            toReturn = StaticPathFinderAgg.mesh.getPathTile(x, y);
            ...
            StaticPathFinderAgg.logger.log(Level.WARNING, "OUT OF BOUNDS AT RAYCAST: " + x + ", " + y + ": " + ai.getMessage(), ai);
            StaticPathFinderAgg.logger.log(Level.WARNING, "Mesh info: " + StaticPathFinderAgg.mesh.getBorderStartX() + ", " + StaticPathFinderAgg.mesh.getBorderStartY() + ", to " + StaticPathFinderAgg.mesh.getBorderEndX() + ", " + StaticPathFinderAgg.mesh.getBorderEndY());
            StaticPathFinderAgg.logger.log(Level.WARNING, "Size of meshx=" + StaticPathFinderAgg.mesh.getSizex() + ", meshy=" + StaticPathFinderAgg.mesh.getSizey());
            throw new NoPathException("Path missed at " + x + ", " + y);
        }
        if (this.stepsTaken > this.maxSteps) {
            if (StaticPathFinderAgg.logger.isLoggable(Level.FINEST)) {
                StaticPathFinderAgg.logger.finest("Raycaster stops searching after " + this.stepsTaken + " steps, suspecting it missed the target.");
            }
    ...
    Path astar(final int startTileX, final int startTileY, final int endTileX, final int endTileY, final boolean aSurfaced) throws NoPathException {
        ...
        if (this.finish == null) {
            if (this.creature != null) {
                StaticPathFinderAgg.logger.log(Level.WARNING, this.creature.getName() + " finish=null at " + endTileX + ", " + endTileY);
            }
            else {
                StaticPathFinderAgg.logger.log(Level.WARNING, "Finish=null at " + endTileX + ", " + endTileY);
            }
            return null;
        }
        if (this.start == null) {
            if (this.creature != null) {
                StaticPathFinderAgg.logger.log(Level.WARNING, this.creature.getName() + " start=null at " + startTileX + ", " + startTileY);
            }
            else {
                StaticPathFinderAgg.logger.log(Level.WARNING, "start=null at " + startTileX + ", " + startTileY);
            }
            return null;
        }
        this.start.setDistanceFromStart(this.start, 0.0f);
        StaticPathFinderAgg.pathList.add(this.start);
        int pass;
        int lState;
        ...
        if (lState == 1) {
            if (pass > 4000) {
                ...
                StaticPathFinderAgg.logger.log(Level.INFO, cname + " pathed from " + this.startX + ", " + this.startY + " to " + this.endX + ", " + this.endY + " and found path after " + pass + " steps.");
            }
            return this.setPath();
        }
        if (lState == 2) {
    ...
    private void setDebug(final boolean aDebug) {
        this.debug = aDebug;
        if (StaticPathFinderAgg.logger.isLoggable(Level.FINEST)) {
            StaticPathFinderAgg.logger.finest("Debug in pathfinding - " + aDebug);
        }
    }
    ...
    int step2() {
        ...
        float score = 0.0f;
        PathTile best = StaticPathFinderAgg.pathList.get(StaticPathFinderAgg.pathList.size() - 1);
        PathTile now = null;
        for (int i = 0; i < StaticPathFinderAgg.pathList.size(); ++i) {
            now = StaticPathFinderAgg.pathList.get(i);
        ...
        now = best;
        StaticPathFinderAgg.pathList.remove(now);
        now.setUsed();
        final PathTile[] next = StaticPathFinderAgg.mesh.getAdjacent(now);
        for (int j = 0; j < next.length; ++j) {
            if (next[j] != null && this.canPass(now, next[j])) {
                if (!StaticPathFinderAgg.pathList.contains(next[j]) && next[j].isNotUsed()) {
                    StaticPathFinderAgg.pathList.add(next[j]);
                }
        ...
        if (StaticPathFinderAgg.pathList.isEmpty()) {
            return 2;
        }
        return 0;
    }
    ...
    Path setPath() {
        ...
        StaticPathFinderAgg.pList = new LinkedList<PathTile>();
        PathTile lastCurrent = now;
        while (!finished) {
            StaticPathFinderAgg.pList.add(now);
            final PathTile next = this.findLowestDist(this.start, now);
            if (lastCurrent.equals(next)) {
                finished = true;
                StaticPathFinderAgg.logger.log(Level.WARNING, "Loop in heuristicastar.");
            }
            lastCurrent = now;
            now = next;
            if (now.equals(stop)) {
        ...
        final Iterator<PathTile> it = StaticPathFinderAgg.pList.iterator();
        while (it.hasNext()) {
