...
final class PathMesh
{
    ...
    PathMesh(final int startx, final int starty, final int endx, final int endy, final boolean surf, final int borderSize, final int[] aMesh) {
        this.surfaced = surf;
        this.borderStartX = Math.max(0, Math.min(startx, endx) - borderSize);
        this.borderEndX = Math.min(PathMesh.WORLD_SIZE - 1, Math.max(startx, endx) + borderSize);
        this.borderStartY = Math.max(0, Math.min(starty, endy) - borderSize);
        this.borderEndY = Math.min(PathMesh.WORLD_SIZE - 1, Math.max(starty, endy) + borderSize);
        this.sizex = this.borderEndX - this.borderStartX + 1;
        this.sizey = this.borderEndY - this.borderStartY + 1;
        if (PathMesh.logger.isLoggable(Level.FINEST)) {
            PathMesh.logger.finest("PathMesh start-end: " + startx + "," + starty + "-" + endx + "," + endy);
            PathMesh.logger.finest("PathMesh border start-end: " + this.borderStartX + ", " + this.borderStartY + " - " + this.borderEndX + ", " + this.borderEndY + ", sz=" + this.sizex + "," + this.sizey);
        }
        ...
        for (int x = 0; x < this.sizex; ++x) {
            for (int y = 0; y < this.sizey; ++y) {
                this.pathables[x][y] = new PathTile(this.borderStartX + x, this.borderStartY + y, aMesh[this.borderStartX + x | this.borderStartY + y << Constants.meshSize], this.surfaced, (byte)(this.surfaced ? 0 : -1));
                if (PathMesh.logger.isLoggable(Level.FINEST)) {
                    PathMesh.logger.finest("pathables: " + this.pathables[x][y].toString());
                }
    ...
    PathMesh(final int startx, final int starty, final int endx, final int endy, final boolean surf, final int borderSize) {
