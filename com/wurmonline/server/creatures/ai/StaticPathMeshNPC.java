...
final class StaticPathMeshNPC
{
    ...
    StaticPathMeshNPC(final int startx, final int starty, final int endx, final int endy, final boolean surf, final int borderSize, final int[] aMesh) {
        this.surfaced = surf;
        this.borderStartX = Math.max(0, Math.min(startx, endx) - borderSize);
        this.borderEndX = Math.min(StaticPathMeshNPC.WORLD_SIZE - 1, Math.max(startx, endx) + borderSize);
        this.borderStartY = Math.max(0, Math.min(starty, endy) - borderSize);
        this.borderEndY = Math.min(StaticPathMeshNPC.WORLD_SIZE - 1, Math.max(starty, endy) + borderSize);
        this.sizex = this.borderEndX - this.borderStartX + 1;
        this.sizey = this.borderEndY - this.borderStartY + 1;
        if (StaticPathMeshNPC.logger.isLoggable(Level.FINEST)) {
            StaticPathMeshNPC.logger.finest("PathMesh start-end: " + startx + "," + starty + "-" + endx + "," + endy);
            StaticPathMeshNPC.logger.finest("PathMesh border start-end: " + this.borderStartX + ", " + this.borderStartY + " - " + this.borderEndX + ", " + this.borderEndY + ", sz=" + this.sizex + "," + this.sizey);
        }
        StaticPathMeshNPC.pathables = new PathTile[this.sizex][this.sizey];
        PathTile lStart = null;
        PathTile lFinish = null;
        for (int x = 0; x < this.sizex; ++x) {
            for (int y = 0; y < this.sizey; ++y) {
                StaticPathMeshNPC.pathables[x][y] = new PathTile(this.borderStartX + x, this.borderStartY + y, aMesh[this.borderStartX + x | this.borderStartY + y << Constants.meshSize], this.surfaced, (byte)(this.surfaced ? 0 : -1));
                if (StaticPathMeshNPC.logger.isLoggable(Level.FINEST)) {
                    StaticPathMeshNPC.logger.finest("pathables: " + StaticPathMeshNPC.pathables[x][y].toString());
                }
                if (StaticPathMeshNPC.pathables[x][y].getTileX() == startx && StaticPathMeshNPC.pathables[x][y].getTileY() == starty) {
                    lStart = StaticPathMeshNPC.pathables[x][y];
                }
                if (StaticPathMeshNPC.pathables[x][y].getTileX() == endx && StaticPathMeshNPC.pathables[x][y].getTileY() == endy) {
                    lFinish = StaticPathMeshNPC.pathables[x][y];
                }
            }
        }
        this.start = lStart;
    ...
    StaticPathMeshNPC(final int startx, final int starty, final int endx, final int endy, final boolean surf, final int borderSize) {
    ...
    PathTile getPathTile(final int realx, final int realy) {
        ...
        return StaticPathMeshNPC.pathables[diffX][diffY];
    }
    ...
    static void clearPathables() {
        StaticPathMeshNPC.pathables = null;
    }
