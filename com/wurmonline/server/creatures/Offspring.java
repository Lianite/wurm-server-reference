...
public final class Offspring
{
    ...
    private static final Map<Long, Offspring> offsprings;
    ...
    private static final String[] MALE_UNI_NAMES;
    private static final String[] FEMALE_UNI_NAMES;
    
    Offspring(final long _motherId, final long _fatherId, final long _traits, final byte _deliveryDays, final boolean load) {
        this.deliveryDays = 0;
        this.checked = false;
        ...
        Offspring.offsprings.put(this.mother, this);
    }
    ...
    private void create() {
        ...
        try {
            ...
            Offspring.logger.log(Level.WARNING, "Failed to create offspring for " + this.mother, ex);
        }
        finally {
    ...
    boolean decreaseDaysLeft() {
        this.checked = true;
        final byte deliveryDays = this.deliveryDays;
        this.deliveryDays = (byte)(deliveryDays - 1);
        if (deliveryDays > 0) {
            Connection dbcon = null;
            PreparedStatement ps = null;
            try {
                ...
                Offspring.logger.log(Level.WARNING, "Failed to update offspring for " + this.mother + ", days=" + this.deliveryDays, ex);
            }
            finally {
                DbUtilities.closeDatabaseObjects(ps, null);
                DbConnector.returnConnection(dbcon);
        ...
        Offspring.offsprings.remove(this.mother);
        deleteSettings(this.mother);
        return true;
    }
    
    static void deleteSettings(final long motherid) {
        Offspring.offsprings.remove(motherid);
        Connection dbcon = null;
        PreparedStatement ps = null;
        try {
            dbcon = DbConnector.getCreatureDbCon();
            ...
            Offspring.logger.log(Level.WARNING, "Failed to delete offspring for " + motherid, ex);
        }
        finally {
            DbUtilities.closeDatabaseObjects(ps, null);
            DbConnector.returnConnection(dbcon);
    ...
    static void loadAllOffspring() {
        ...
        try {
            ...
            Offspring.logger.log(Level.WARNING, "Failed loading all offspring " + ex.getMessage(), ex);
        }
        finally {
    ...
    static Offspring getOffspring(final long wurmid) {
        return Offspring.offsprings.get(wurmid);
    }
    ...
    public static void resetOffspringCounters() {
        final ArrayList<Offspring> invalid = new ArrayList<Offspring>();
        for (final Offspring lOffspring : Offspring.offsprings.values()) {
            if (Creatures.getInstance().getCreatureOrNull(lOffspring.mother) == null) {
                invalid.add(lOffspring);
            }
            lOffspring.checked = false;
        }
        for (final Offspring offspring : invalid) {
            Offspring.logger.warning("Deleting offspring data, mother not found. mother=" + offspring.mother + ", father=" + offspring.father + ", traits=" + offspring.traits + ", deliveryDays=" + offspring.deliveryDays);
    ...
    static String generateMaleName() {
        if (Offspring.MALE_NAMES.length <= 0) {
            return "";
        }
        ...
        if (num < 50) {
            return generateGenericPrefix() + Offspring.MALE_NAMES[Server.rand.nextInt(Offspring.MALE_NAMES.length)];
        }
        return Offspring.MALE_NAMES[Server.rand.nextInt(Offspring.MALE_NAMES.length)] + generateGenericPrefix();
    }
    
    static String generateGenericPrefix() {
        if (Offspring.GENERIC_NAMES.length > 0) {
            return Offspring.GENERIC_NAMES[Server.rand.nextInt(Offspring.GENERIC_NAMES.length)];
        }
        return "";
    }
    ...
    static String generateGenericName() {
        if (Offspring.GENERIC_NAMES.length > 0) {
            return Offspring.GENERIC_NAMES[Server.rand.nextInt(Offspring.GENERIC_NAMES.length)] + Offspring.GENERIC_NAMES[Server.rand.nextInt(Offspring.GENERIC_NAMES.length)];
        }
        return "";
    }
    
    static String generateFemaleName() {
        if (Offspring.FEMALE_NAMES.length <= 0) {
            return "";
        }
        final int num = Server.rand.nextInt(100);
        if (num == 99) {
        ...
        if (num < 50) {
            return generateGenericPrefix() + Offspring.FEMALE_NAMES[Server.rand.nextInt(Offspring.FEMALE_NAMES.length)];
        }
        return Offspring.FEMALE_NAMES[Server.rand.nextInt(Offspring.FEMALE_NAMES.length)] + generateGenericPrefix();
    }
    
    static String generateFemaleUnicornName() {
        if (Offspring.FEMALE_UNI_NAMES.length <= 0) {
            return "";
        }
        final int num = Server.rand.nextInt(100);
        if (num == 99) {
            return Offspring.FEMALE_UNI_NAMES[Server.rand.nextInt(Offspring.FEMALE_UNI_NAMES.length)];
        }
        if (num < 50) {
            return generateGenericPrefix() + Offspring.FEMALE_UNI_NAMES[Server.rand.nextInt(Offspring.FEMALE_UNI_NAMES.length)];
        }
        return Offspring.FEMALE_UNI_NAMES[Server.rand.nextInt(Offspring.FEMALE_UNI_NAMES.length)] + generateGenericPrefix();
    }
    
    static String generateMaleUnicornName() {
        if (Offspring.MALE_UNI_NAMES.length <= 0) {
            return "";
        }
        final int num = Server.rand.nextInt(100);
        if (num == 99) {
            return Offspring.MALE_UNI_NAMES[Server.rand.nextInt(Offspring.MALE_UNI_NAMES.length)];
        }
        if (num < 50) {
            return generateGenericPrefix() + Offspring.MALE_UNI_NAMES[Server.rand.nextInt(Offspring.MALE_UNI_NAMES.length)];
        }
        return Offspring.MALE_UNI_NAMES[Server.rand.nextInt(Offspring.MALE_UNI_NAMES.length)] + generateGenericPrefix();
    }
    ...
    static String[] createMaleNames() {
        final List<String> malelist = new ArrayList<String>(80);
        malelist.add("lars");
        malelist.add("bom");
        malelist.add("ally");
        malelist.add("dom");
    ...
    static String[] createFemaleNames() {
        final List<String> femalelist = new ArrayList<String>(80);
    ...
    static String[] createGenericNames() {
        final List<String> genericlist = new ArrayList<String>(59);
    ...
    private static String[] createMaleUnicornNames() {
        final List<String> mul = new ArrayList<String>();
        mul.add("Amor");
        mul.add("Amulius");
        mul.add("Aries");
        mul.add("Belial");
    ...
    private static String[] createFemaleUnicornNames() {
        final List<String> ful = new ArrayList<String>();
        ful.add("Amor");
        ful.add("Angerona");
        ful.add("Aurora");
        ful.add("Bellona");
    ...
    static {
        logger = Logger.getLogger(Offspring.class.getName());
        offsprings = new ConcurrentHashMap<Long, Offspring>();
        MALE_NAMES = createMaleNames();
        FEMALE_NAMES = createFemaleNames();
        GENERIC_NAMES = createGenericNames();
        MALE_UNI_NAMES = createMaleUnicornNames();
        FEMALE_UNI_NAMES = createFemaleUnicornNames();
    }
}
