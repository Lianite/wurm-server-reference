...
public class NonRegisteringDriver implements Driver
{
    ...
    private Connection connectLoadBalanced(final String url, final Properties info) throws SQLException {
        ...
        return (Connection)Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class[] { (NonRegisteringDriver.class$java$sql$Connection == null) ? (NonRegisteringDriver.class$java$sql$Connection = class$("java.sql.Connection")) : NonRegisteringDriver.class$java$sql$Connection }, proxyBal);
    }
    ...
    public Properties parseURL(String url, final Properties defaults) throws SQLException {
        ...
        if (hostStuff != null && hostStuff.trim().length() > 0) {
            final StringTokenizer st = new StringTokenizer(hostStuff, ",");
            while (st.hasMoreTokens()) {
                ++numHosts;
                final String[] hostPortPair = parseHostPortPair(st.nextToken());
