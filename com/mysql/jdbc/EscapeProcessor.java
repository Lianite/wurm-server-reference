...
class EscapeProcessor
{
    ...
    public static final Object escapeSQL(final String sql, final boolean serverSupportsConvertFn, final ConnectionImpl conn) throws SQLException {
    ...
    private static void processTimeToken(final ConnectionImpl conn, final StringBuffer newSql, final String token) throws SQLException {
        final int startPos = token.indexOf(39) + 1;
        final int endPos = token.lastIndexOf(39);
        if (startPos == -1 || endPos == -1) {
            newSql.append(token);
    ...
    private static void processTimestampToken(final ConnectionImpl conn, final StringBuffer newSql, final String token) throws SQLException {
        final int startPos = token.indexOf(39) + 1;
        final int endPos = token.lastIndexOf(39);
        if (startPos == -1 || endPos == -1) {
            newSql.append(token);
    ...
    private static String processConvertToken(final String functionToken, final boolean serverSupportsConvertFn, final ConnectionImpl conn) throws SQLException {
