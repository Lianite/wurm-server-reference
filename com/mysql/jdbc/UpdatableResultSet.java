...
public class UpdatableResultSet extends ResultSetImpl
{
    ...
    protected UpdatableResultSet(final String catalog, final Field[] fields, final RowData tuples, final ConnectionImpl conn, final StatementImpl creatorStmt) throws SQLException {
    ...
    public synchronized void deleteRow() throws SQLException {
        ...
        if (numKeys == 1) {
            ...
            final byte[] currentVal = this.thisRow.getColumnValue(index);
            this.deleter.setBytes(1, currentVal);
        }
        else {
            for (int i = 0; i < numKeys; ++i) {
                final int index2 = this.primaryKeyIndicies.get(i);
                final byte[] currentVal2 = this.thisRow.getColumnValue(index2);
                this.deleter.setBytes(i + 1, currentVal2);
            }
        }
    ...
    protected synchronized void syncUpdate() throws SQLException {
        ...
        if (numKeys == 1) {
            final int index = this.primaryKeyIndicies.get(0);
            final byte[] keyData = this.thisRow.getColumnValue(index);
            this.updater.setBytes(numFields + 1, keyData, false, false);
        }
        else {
            for (int j = 0; j < numKeys; ++j) {
                final byte[] currentVal = this.thisRow.getColumnValue(this.primaryKeyIndicies.get(j));
                if (currentVal != null) {
                    this.updater.setBytes(numFields + j + 1, currentVal, false, false);
                }
                else {
                    this.updater.setNull(numFields + j + 1, 0);
                }
            }
        }
    }
