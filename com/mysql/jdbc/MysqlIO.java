...
class MysqlIO
{
    ...
    public MysqlIO(final String host, final int port, final Properties props, final String socketFactoryClassName, final ConnectionImpl conn, final int socketTimeout, final int useBufferRowSizeThreshold) throws IOException, SQLException {
    ...
    final ResultSetInternalMethods sqlQueryDirect(final StatementImpl callingStatement, final String query, final String characterEncoding, Buffer queryPacket, final int maxRows, final int resultSetType, final int resultSetConcurrency, final boolean streamResults, final String catalog, final Field[] cachedMetadata) throws Exception {
        ...
        try {
            if (this.statementInterceptors != null) {
                final ResultSetInternalMethods interceptedResults = this.invokeStatementInterceptorsPre(query, callingStatement);
            ...
            if (this.statementInterceptors != null) {
                final ResultSetInternalMethods interceptedResults2 = this.invokeStatementInterceptorsPost(query, callingStatement, rs);
    ...
    private ResultSetInternalMethods invokeStatementInterceptorsPre(final String sql, final com.mysql.jdbc.Statement interceptedStatement) throws SQLException {
        ResultSetInternalMethods previousResultSet = null;
        for (final StatementInterceptor interceptor : this.statementInterceptors) {
            final boolean executeTopLevelOnly = interceptor.executeTopLevelOnly();
            final boolean shouldExecute = (executeTopLevelOnly && this.statementExecutionDepth == 1) || !executeTopLevelOnly;
            if (shouldExecute) {
    ...
    private ResultSetInternalMethods invokeStatementInterceptorsPost(final String sql, final com.mysql.jdbc.Statement interceptedStatement, ResultSetInternalMethods originalResultSet) throws SQLException {
        for (final StatementInterceptor interceptor : this.statementInterceptors) {
            final boolean executeTopLevelOnly = interceptor.executeTopLevelOnly();
            final boolean shouldExecute = (executeTopLevelOnly && this.statementExecutionDepth == 1) || !executeTopLevelOnly;
            if (shouldExecute) {
                ...
                final ResultSetInternalMethods interceptedResultSet = interceptor.postProcess(sqlToInterceptor, interceptedStatement, originalResultSet, this.connection);
                if (interceptedResultSet == null) {
                    continue;
                }
    ...
    private void setServerSlowQueryFlags() {
        if (this.profileSql) {
            this.queryNoIndexUsed = ((this.serverStatus & 0x10) != 0x0);
            this.queryBadIndexUsed = ((this.serverStatus & 0x20) != 0x0);
    ...
    private final Buffer reuseAndReadPacket(final Buffer reuse, final int existingPacketLength) throws SQLException {
        try {
            ...
            if (packetLength == this.maxThreeBytes) {
                ...
                final int packetEndPoint = packetLength;
                isMultiPacket = true;
                packetLength = this.readRemainingMultiPackets(reuse, multiPacketSeq, packetEndPoint);
            }
            if (!isMultiPacket) {
    ...
    private void checkErrorPacket(final Buffer resultPacket) throws SQLException {
        ...
        if (this.protocolVersion > 9) {
            ...
            throw SQLError.createSQLException(errorBuf.toString(), xOpen, errno, this.getExceptionInterceptor());
        }
        else {
            ...
            if (serverErrorMessage.indexOf(Messages.getString("MysqlIO.70")) != -1) {
                throw SQLError.createSQLException(SQLError.get("S0022") + ", " + serverErrorMessage, "S0022", -1, this.getExceptionInterceptor());
            }
            final StringBuffer errorBuf2 = new StringBuffer(Messages.getString("MysqlIO.72"));
            errorBuf2.append(serverErrorMessage);
            errorBuf2.append("\"");
            throw SQLError.createSQLException(SQLError.get("S1000") + ", " + errorBuf2.toString(), "S1000", -1, this.getExceptionInterceptor());
        }
    }
