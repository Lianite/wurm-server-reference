...
public abstract class JDBC3ResultSet extends CoreResultSet
{
    ...
    public InputStream getBinaryStream(final int col) throws SQLException {
        return new ByteArrayInputStream(this.getBytes(col));
    }
    ...
    public Date getDate(final int col, final Calendar cal) throws SQLException {
        ...
        switch (this.db.column_type(this.stmt.pointer, this.markCol(col))) {
            ...
            case 3: {
                try {
                    final FastDateFormat dateFormat = FastDateFormat.getInstance(this.stmt.conn.dateStringFormat, cal.getTimeZone());
    ...
    public int getColumnType(final int col) throws SQLException {
        ...
        final int valueType = this.db.column_type(this.stmt.pointer, this.checkCol(col));
    ...
    private String getColumnDeclType(final int col) throws SQLException {
        ...
        if (declType == null) {
            final Matcher matcher = JDBC3ResultSet.COLUMN_TYPECAST.matcher(this.db.column_name(this.stmt.pointer, this.checkCol(col)));
