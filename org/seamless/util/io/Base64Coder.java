...
public class Base64Coder
{
    ...
    private static final byte[] getAlphabet(final int options) {
        if ((options & 0x10) == 0x10) {
            return Base64Coder._URL_SAFE_ALPHABET;
        }
        if ((options & 0x20) == 0x20) {
            return Base64Coder._ORDERED_ALPHABET;
        }
        return Base64Coder._STANDARD_ALPHABET;
    }
    
    private static final byte[] getDecodabet(final int options) {
        if ((options & 0x10) == 0x10) {
            return Base64Coder._URL_SAFE_DECODABET;
        }
        if ((options & 0x20) == 0x20) {
            return Base64Coder._ORDERED_DECODABET;
        }
        return Base64Coder._STANDARD_DECODABET;
    }
    ...
    public static String encodeFromFile(final String filename) throws IOException {
        ...
        try {
            ...
            int length;
            int numBytes;
            for (buffer = new byte[Math.max((int)(file.length() * 1.4 + 1.0), 40)], length = 0, numBytes = 0, bis = new InputStream(new BufferedInputStream(new FileInputStream(file)), 1); (numBytes = bis.read(buffer, length, 4096)) >= 0; length += numBytes) {}
