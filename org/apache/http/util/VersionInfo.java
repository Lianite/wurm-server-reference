...
public class VersionInfo
{
    ...
    public static final VersionInfo[] loadVersionInfo(final String[] pckgs, final ClassLoader clsldr) {
        if (pckgs == null) {
        ...
        for (int i = 0; i < pckgs.length; ++i) {
            final VersionInfo vi = loadVersionInfo(pckgs[i], clsldr);
            if (vi != null) {
                vil.add(vi);
            }
        }
    ...
    public static final VersionInfo loadVersionInfo(final String pckg, ClassLoader clsldr) {
        if (pckg == null) {
            throw new IllegalArgumentException("Package identifier must not be null.");
        }
        if (clsldr == null) {
        ...
        Properties vip = null;
        try {
            final InputStream is = clsldr.getResourceAsStream(pckg.replace('.', '/') + "/" + "version.properties");
            if (is != null) {
                try {
    ...
    protected static final VersionInfo fromMap(final String pckg, final Map<?, ?> info, final ClassLoader clsldr) {
        if (pckg == null) {
            throw new IllegalArgumentException("Package identifier must not be null.");
        }
        String module = null;
