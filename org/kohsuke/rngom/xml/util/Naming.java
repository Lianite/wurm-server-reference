...
public class Naming
{
    ...
    private static void setCharType(final char c, final int type) {
        final int hi = c >> 8;
        if (Naming.charTypeTable[hi] == null) {
            Naming.charTypeTable[hi] = new byte[256];
        }
        Naming.charTypeTable[hi][c & '\u00ff'] = (byte)type;
    }
    
    private static void setCharType(char min, final char max, final int type) {
        byte[] shared = null;
        char c;
        do {
            if ((min & '\u00ff') == '\0') {
                while (min + '\u00ff' <= max) {
                    ...
                    Naming.charTypeTable[min >> 8] = shared;
                    if (min + '\u00ff' == max) {
                        return;
                    }
                    min += '\u0100';
    ...
    private static boolean isNameStartChar(final char c) {
        return Naming.charTypeTable[c >> 8][c & '\u00ff'] == 2;
    }
    
    private static boolean isNameStartCharNs(final char c) {
        return isNameStartChar(c) && c != ':';
    }
    
    private static boolean isNameChar(final char c) {
        return Naming.charTypeTable[c >> 8][c & '\u00ff'] != 0;
    }
    
    private static boolean isNameCharNs(final char c) {
    ...
    public static boolean isNmtoken(final String s) {
        final int len = s.length();
        if (len == 0) {
            return false;
        }
    ...
    static {
        ...
        for (int j = 0; j < 256; ++j) {
            if (Naming.charTypeTable[j] == null) {
                Naming.charTypeTable[j] = other;
            }
        }
    }
}
